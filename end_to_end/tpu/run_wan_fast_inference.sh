...
#   FIXEDM=0         plain online softmax instead of fixed-m
#   EXTRA_LIBTPU     extra libtpu flags, appended to the tuned set
#
# 720p 81f / 40 steps denoise: fixed-m 105.3s, plain 109.6s. Each mode gets its
# own optimal tile below; plain degrades badly on fixed-m's.
set -u
MODEL=${1:-22}
STEPS=${2:-40}
PROMPT=${3:-""}
...
# Attention selection
#
# Default kernel configuration uses internal 2D mesh sharding with per-Q-block
# fixed-m metadata.
# Topology note: with ici_context_parallelism=4 and ulysses_shards=4, ring degree is 1
# (R = CP // U = 1), so no KV ring rotations occur; execution is 1D Ulysses attention
# over the internal 2D mesh without inter-chip KV transfers.
#
# Measured benchmarks on Cloud TPU v6e-8 (Wan 2.2 T2V-A14B, 720p, 81f, 40 steps):
# Shipped default recipe (tile BQ=9472 / BKV=1024, U=4, CP=4, DP=2):
...
# (Note: earlier exploratory benchmarks reported 137.7s -> 137.5s during an intermediate AdaLN ablation experiment).
#
# Comparative tile size benchmark (tile BQ=6400 / BKV=1024, U=4, 25 prompts):
#   ulysses_ring_custom_fixed_m_per_q_block: 3.4919 s/step (-1.01% vs 3.5274s)

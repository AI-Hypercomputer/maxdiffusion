...
export LOCAL_IMAGE_NAME=maxdif_base_image
...
if [[ -z ${LIBTPU_GCS_PATH+x} ]] ; then
  ...
  if [[ ${DEVICE} == "gpu" ]]; then
    ...
    docker build --network host --build-arg MODE=${MODE} --build-arg JAX_VERSION=$JAX_VERSION --build-arg LIBTPU_GCS_PATH=$LIBTPU_GCS_PATH --build-arg DEVICE=$DEVICE -f ./maxdif_dependencies.Dockerfile -t ${LOCAL_IMAGE_NAME} .
  fi
else
  docker build --network host --build-arg MODE=${MODE} --build-arg JAX_VERSION=$JAX_VERSION --build-arg LIBTPU_GCS_PATH=$LIBTPU_GCS_PATH -f ./maxdif_dependencies.Dockerfile -t ${LOCAL_IMAGE_NAME} .
  docker build --network host --build-arg CUSTOM_LIBTPU=true -f ./maxdif_libtpu_path.Dockerfile -t ${LOCAL_IMAGE_NAME} .
fi

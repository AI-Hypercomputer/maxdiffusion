...
class CNN(nn.Module):
  ...
  aqt_cfg_conv: aqt_config.DotGeneralRaw
  ...
  def __call__(self, x):
    ...
    aqt_cfg_conv = aqt_cfg_conv.replace(fwd=self.aqt_cfg_conv)
    aqt_conv = functools.partial(
    ...
    x = nn.Conv(
        features=32, kernel_size=(3, 3), conv_general_dilated_cls=aqt_conv
        )(x)
    x = nn.BatchNorm(use_running_average=use_running_avg, dtype=x.dtype)(x)
    x = nn.relu(x)
    x = nn.avg_pool(x, window_shape=(2, 2), strides=(2, 2))
    x = nn.Conv(
        features=64, kernel_size=(3, 3), conv_general_dilated_cls=aqt_conv
        )(x)
    x = nn.BatchNorm(use_running_average=use_running_avg, dtype=x.dtype)(x)
    x = nn.relu(x)
    x = nn.avg_pool(x, window_shape=(2, 2), strides=(2, 2))
...
def apply_model(model_params, images, labels, apply_fn):
  ...
  # print('SS apply_model grads', grads['params']['Conv_0']['kernel'][:,:,:, 0],
  #       flush=True)
  loss, (logits, updated_var) = aux
  # print('SS apply_model updated_var',
  #       updated_var['params']['Conv_0']['kernel'][:,:,:, 0],
  #       flush=True)
...
def train_epoch(state, train_ds, batch_size, rng):
  ...
  # print('SS ======== train_epoch perms', len(perms))
  for perm in perms:
    # print('SS ======= train_epoch perm', perm)
    batch_images = train_ds['image'][perm, ...]
    batch_labels = train_ds['label'][perm, ...]
    grads, loss, accuracy, updated_var = apply_model(
        state.model, batch_images, batch_labels, state.cnn_train.apply
    )
    # print('SS ======== train_epoch apply_model updated_var',
    #       updated_var['params']['Conv_0']['kernel'][:,:,:, 0], flush=True)
    state = update_model(state, grads, updated_var)
    # print('SS ======== train_epoch new state',
    #       state.model['params']['Conv_0']['kernel'][:,:,:, 0], flush=True)
...
def create_train_state(rng, aqt_cfg, aqt_cfg_conv):
  """Creates initial `TrainState`."""
  cnn_train = CNN(
      bn_use_stats=True, aqt_cfg=aqt_cfg, aqt_cfg_conv=aqt_cfg_conv)
  model = cnn_train.init({'params': rng}, jnp.ones([1, 28, 28, 1]))
  learning_rate = 0.1
  momentum = 0.9
  tx = optax.sgd(learning_rate, momentum)
  cnn_eval = CNN(
      bn_use_stats=False, aqt_cfg=aqt_cfg, aqt_cfg_conv=aqt_cfg_conv)
  return TrainState(
      cnn_train=cnn_train,
      cnn_eval=cnn_eval,
      model=model,
...
def train_and_evaluate(
  ...
  if state is None:
    ...
    state = create_train_state(init_rng, aqt_cfg, aqt_cfg_conv)
...
def main(argv):
  ...
  aqt_cfg_conv = aqt_config.conv_general_dilated_make(
  ...
  update_cfg_raw_with_calibration(state.cnn_eval.aqt_cfg_conv)
  ...
  print(jax.tree.map(lambda x: x.shape, state.model))

...
from maxdiffusion.max_utils import (create_device_mesh, get_abstract_state, setup_initial_state)
...
def run(config):
  ...
  unboxed_abstract_state, state_mesh_annotations = get_abstract_state(
      unet, None, config, mesh, unet_variables, training=False
  )
  ...
  state, state_mesh_shardings = setup_initial_state(
      unet, None, config, mesh, rng, params, unboxed_abstract_state, state_mesh_annotations, training=False
  )

...
def _compute_p_and_ds(qk_slice, v_slice, do, lse, di, exp_fn):
  """Computes attention weights p and gradient ds for a KV chunk."""
  p_curr = exp_fn(qk_slice - lse)
  ...
  return p_curr, ds_curr


def _compute_dq_tile(ds_curr, k_slice):
  """Computes dQ gradient tile for a given dS and K chunk."""
...
def _accumulate_dk_dv(sub_slice, p_curr, ds_curr, q, do, dk_scratch_ref, dv_scratch_ref):
  """Accumulates gradient contributions for dK and dV tiles into scratch references."""
  float32 = jnp.float32
...
def _run_kv_compute_loop(
  ...
  """Executes the loop over KV chunks with exact tail handling."""
  assert bkv % bkv_compute == 0
...
def _flash_attention_dq_kernel(
  ...
  def _dq_inner(qk, k_chunk, v_chunk, do, lse, di, dq_prev):
    ...
    for idx in range(0, qk.shape[0], step):
      ...
      _, ds_curr = _compute_p_and_ds(qk_slice, v_slice, do, lse, di, exp)
      dq_prev = dq_prev + _compute_dq_tile(ds_curr, k_slice)
    return dq_prev
  ...
  _run_kv_compute_loop(
      j, grid_width, bkv, bkv_compute, kv_seq_len, compute_body, last_compute_body
  )
...
def _flash_attention_dkv_kernel(
  ...
  def _dkv_inner(base_offset, qk, q, v_chunk, do, lse, di):
    ...
    for idx in range(0, qk.shape[0], step):
      ...
      p_curr, ds_curr = _compute_p_and_ds(qk_slice, v_slice, do, lse, di, exp)
      _accumulate_dk_dv(sub_slice, p_curr, ds_curr, q, do, dk_scratch_ref, dv_scratch_ref)
...
def _flash_attention_bwd_fused_kernel(
  ...
  def _bwd_fused_inner(base_offset, qk, k_chunk, v_chunk, q, do, lse, di):
    ...
    for idx in range(0, qk.shape[0], step):
      ...
      p_curr, ds_curr = _compute_p_and_ds(qk_slice, v_slice, do, lse, di, exp)
      _accumulate_dk_dv(sub_slice, p_curr, ds_curr, q, do, dk_scratch_ref, dv_scratch_ref)
      dq_acc = dq_acc + _compute_dq_tile(ds_curr, k_slice)
...
def _splash_attention_bwd_fused(
  ...
  if use_dq_aliasing:
    def dq_index_map_aliased(j, h_q, i, *_):
      return (j % 3, h_q, i, 0)

    dq_spec = pl.BlockSpec((None, None, bq, head_dim_qk), dq_index_map_aliased)
    dq_alias_spec = dq_spec
    dq_dtype = jnp.float32
    dq_shape = jax.ShapeDtypeStruct((3, num_q_heads, active_q_len, head_dim_qk), dq_dtype)
    dq_init = jnp.zeros((3, num_q_heads, active_q_len, head_dim_qk), dtype=dq_dtype)
  else:
    def dq_index_map_standard(j, h_q, i, *_):
      return (j, h_q, i, 0)

    dq_spec = pl.BlockSpec((None, None, bq, head_dim_qk), dq_index_map_standard)
    dq_alias_spec = None
...
def _splash_attention_custom(
  ...
  if residual_checkpoint_name is not None:
    out = jax.ad_checkpoint.checkpoint_name(out, name=residual_checkpoint_name)
  return out
...
def _splash_attention_fwd(
  ...
  if residual_checkpoint_name is not None:
    out = jax.ad_checkpoint.checkpoint_name(out, name=residual_checkpoint_name)
    lse = jax.ad_checkpoint.checkpoint_name(lse, name=residual_checkpoint_name)

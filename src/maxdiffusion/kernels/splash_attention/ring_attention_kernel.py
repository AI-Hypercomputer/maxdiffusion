    ...
    def _accumulate_scan(_):
      ...
      for hop in range(ring_size):
        ...
        o_curr, _, l_curr = custom_splash._splash_attention_forward_ring(  # pylint: disable=protected-access
            ...
            mk=mk_arr,
            k_mean=k_mean,
            # This branch runs under `all_fixed_global` (or when the caller
            # forces it with `uniform_fixed_m=True`), so the kernel is
...
def make_custom_ring_attention(
  ...
  kernel gates in squared-norm space (`|q|^2 * R_k^2 <= floor(W/2)^2`), which is the
  ...
  **required** when `use_fixed_m=True`: unlike the Cauchy-Schwarz norm bounds it
  ...
  overflow (float16 with Q=K=0, V=1 yields inf). With the default
  `uniform_fixed_m=None`, pass `v_ok=False` to force the online fallback.

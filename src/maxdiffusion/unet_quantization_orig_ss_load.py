...
def get_unet_inputs(rng, config, batch_size, pipeline, params, data_sharding):
...
def run_inference(unet_state, vae_state, params, rng, config, batch_size, pipeline, mesh, data_sharding):
  ...
  guidance_scale,
  guidance_rescale,
  scheduler_state) = get_unet_inputs(
    rng, config, batch_size, pipeline, params, data_sharding)

...
class FlaxFlux2KleinProfilePipeline(FlaxDiffusionPipeline):
  """
  Dedicated profiling pipeline for Flux.2-klein-4B and 9B models on JAX+TPU.
  Wraps individual components in isolated jax.profiler.start_trace/stop_trace sessions.
  """
  ...
  def __call__(
      ...
      profile_target: Optional[str] = "all",
    ...
    with self.mesh, nn_partitioning.axis_rules(self._config.logical_axis_rules):
      ...
      tb_dir = tensorboard_dir or getattr(self._config, "tensorboard_dir", "/mnt/workspace/maxdiffusion/tensorboard_traces")
      ...
      # ---------------------------------------------------------------------
      if images is not None and len(images) > 0:
        do_prof_vae_enc = profile_target in ("all", "vae_encode")
        if do_prof_vae_enc and not warmup:
          trace_path = os.path.join(tb_dir, "profile_vae_encode")
          os.makedirs(trace_path, exist_ok=True)
          max_logging.log(f"🚀 [PROFILER] Starting XProf trace for VAE Reference Encode -> {trace_path}")
        ...
        if do_prof_vae_enc and not warmup:
          ...
          max_logging.log("✅ [PROFILER] Completed XProf trace for VAE Reference Encode")
      ...
      # ---------------------------------------------------------------------
      if not prompts:
        raise ValueError("Prompt must be provided to FlaxFlux2KleinProfilePipeline")
      ...
      try:
        ...
        if do_prof_qwen3 and not warmup:
          trace_path = os.path.join(tb_dir, "profile_qwen3")
          os.makedirs(trace_path, exist_ok=True)
          max_logging.log(f"🚀 [PROFILER] Starting XProf trace for Qwen3 Text Encoder -> {trace_path}")
          jax.profiler.start_trace(trace_path)
        ...
        if do_prof_qwen3 and not warmup:
          jax.profiler.stop_trace()
          max_logging.log("✅ [PROFILER] Completed XProf trace for Qwen3 Text Encoder")
      ...
      try:
        ...
        if do_prof_denoise and not warmup:
          trace_path = os.path.join(tb_dir, "profile_denoise")
          os.makedirs(trace_path, exist_ok=True)
          max_logging.log(f"🚀 [PROFILER] Starting XProf trace for Flux Denoising Loop -> {trace_path}")
          jax.profiler.start_trace(trace_path)
        ...
        if do_prof_denoise and not warmup:
          jax.profiler.stop_trace()
          max_logging.log("✅ [PROFILER] Completed XProf trace for Flux Denoising Loop")
    ...
    if do_prof_vae and not warmup:
      trace_path = os.path.join(tb_dir, "profile_vae")
      os.makedirs(trace_path, exist_ok=True)
      max_logging.log(f"🚀 [PROFILER] Starting XProf trace for VAE Decoder -> {trace_path}")
      jax.profiler.start_trace(trace_path)
    ...
    if do_prof_vae and not warmup:
      jax.profiler.stop_trace()
      max_logging.log("✅ [PROFILER] Completed XProf trace for VAE Decoder")

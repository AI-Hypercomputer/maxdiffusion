...
class FlaxFlux2KleinPipeline(FlaxDiffusionPipeline):
  ...
  def _setup_jit_functions(self):
    ...
    if isinstance(self.transformer, nnx.Module):
      graphdef, _, rest = nnx.split(self.transformer, nnx.Param, ...)

      @jax.jit
      def transformer_step_nnx(t_params, latents, img_ids, prompt_embeds, txt_ids, vec, timestep, guidance):
        transformer = nnx.merge(graphdef, t_params, rest)
        out = transformer(
      ...
      self._jitted_transformer_step = transformer_step_nnx
    else:
  ...
  def compile_aot_async(
      self, params, vae_params, qwen3_params, vae_bn_mean, vae_bn_std, batch_size=1, height=1024, width=1024
  ):

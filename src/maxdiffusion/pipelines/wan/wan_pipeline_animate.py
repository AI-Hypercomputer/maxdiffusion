...
@partial(jax.jit, static_argnames=("batch_size", "motion_encode_batch_size"))
def animate_encode_face_motion(
    graphdef,
    sharded_state,
    rest_of_state,
  ...
  """Pre-compute face motion vectors once per segment (before the denoising loop)."""
  wan_transformer = nnx.merge(graphdef, sharded_state, rest_of_state)
  return wan_transformer.encode_face_motion(face_video_segment, batch_size, motion_encode_batch_size)
...
def animate_transformer_forward_pass(
  ...
  Args:
    ...
    pose_latents: VAE-encoded pose video, shape (B, T_lat, H_lat, W_lat, z_dim),
      channel-last.
    face_motion_vec: Pre-computed face motion vectors from encode_face_motion.
  ...
  def __call__(
    ...
    if do_classifier_free_guidance:
      if negative_prompt_embeds is None:
        raise ValueError("`negative_prompt_embeds` must be available when classifier-free guidance is enabled.")
      transformer_prompt_embeds = jnp.concatenate([prompt_embeds, negative_prompt_embeds], axis=0)
      transformer_image_embeds = jnp.concatenate([image_embeds, image_embeds], axis=0)
    else:
      transformer_prompt_embeds = prompt_embeds
      transformer_image_embeds = image_embeds
    ...
    transformer_prompt_embeds = jax.device_put(transformer_prompt_embeds, data_sharding)
    transformer_image_embeds = jax.device_put(transformer_image_embeds, data_sharding)
    ...
    for _seg in range(num_segments):
      ...
      # Pre-compute face motion vectors once per segment (batched for CFG).
      with self.mesh, nn_partitioning.axis_rules(self.config.logical_axis_rules):
        if do_classifier_free_guidance:
          face_seg_uncond = face_seg * 0 - 1
          face_seg_batched = jnp.concatenate([face_seg, face_seg_uncond], axis=0)
        else:
          face_seg_batched = face_seg
        model_face_motion = animate_encode_face_motion(
            graphdef,
            state,
            rest_of_state,
            face_seg_batched,
            face_seg_batched.shape[0],
      ...
      with self.mesh, nn_partitioning.axis_rules(self.config.logical_axis_rules):
        for step in range(num_inference_steps):
          ...
          if do_classifier_free_guidance:
            model_latents = jnp.concatenate([seg_latents, seg_latents], axis=0)
            model_reference_latents = jnp.concatenate([reference_latents, reference_latents], axis=0)
            model_pose_latents = jnp.concatenate([pose_latents, pose_latents], axis=0)
            model_timestep = jnp.concatenate([timestep, timestep], axis=0)
          else:
            model_latents = seg_latents
            model_reference_latents = reference_latents
            model_pose_latents = pose_latents
          ...
          noise_pred = animate_transformer_forward_pass(
              ...
              rest_of_state,
              model_latents,
              model_reference_latents,
              model_pose_latents,
              model_face_motion,
              model_timestep,
              transformer_prompt_embeds,
              transformer_image_embeds,
          )

          if do_classifier_free_guidance:
            noise_cond = noise_pred[:effective_batch_size]
            noise_uncond = noise_pred[effective_batch_size:]

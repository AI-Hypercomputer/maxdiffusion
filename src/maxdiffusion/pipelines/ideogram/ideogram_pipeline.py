...
from ...models.ideogram.qwen3_text_encoder import Qwen3VLTextEncoder
...
class IdeogramPipeline:
  ...
  def _load_and_init(cls, config, restored_checkpoint, vae_only=False, load_transformer=True):
    ...
    if load_transformer:
      ...
      transformer_config = Ideogram4Config(dtype=dtype)

      # Load Conditional Transformer
      conditional_transformer = nnx.eval_shape(
          lambda rngs: Ideogram4Transformer(rngs, transformer_config, dtype=dtype), rngs
      )
      ...
      conditional_transformer = Ideogram4Transformer(rngs, transformer_config, dtype=dtype)
      ...
      unconditional_transformer = Ideogram4Transformer(rngs, transformer_config, dtype=dtype)
    ...
    text_encoder = Qwen3VLTextEncoder.from_pretrained(text_encoder_repo, subfolder=subfolder)

    from transformers import AutoTokenizer
    import os

    tokenizer_repo_or_path = config.pretrained_model_name_or_path
    if os.path.isdir(tokenizer_repo_or_path):
      tokenizer_path = os.path.join(tokenizer_repo_or_path, "tokenizer")
      if not os.path.isdir(tokenizer_path):
        tokenizer_path = tokenizer_repo_or_path
    else:
      try:
        from huggingface_hub import snapshot_download

        cached_repo_dir = snapshot_download(tokenizer_repo_or_path, local_files_only=True)
        tokenizer_path = os.path.join(cached_repo_dir, "tokenizer")
      except Exception:
        tokenizer_path = tokenizer_repo_or_path

    tokenizer = AutoTokenizer.from_pretrained(tokenizer_path, extra_special_tokens={})
  ...
  def _build_inputs_cpu(self, prompts, height, width):
    ...
    for prompt in prompts:
      ...
      if self.tokenizer is not None and prompt != "":
  ...
  def generate(
    ...
    # 1. Text Encoding (using Qwen3-VL text encoder)
    ...
    sigmas = schedule_fn(step_intervals)

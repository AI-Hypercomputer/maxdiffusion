...
class ZImagePipeline:
  ...
  def __init__(
    ...
    # FlowMatchEulerDiscreteScheduler's `shift`: 3.0 Turbo, 6.0 base.
  ...
  def encode_prompt(self, prompt: Union[str, list[str]], max_sequence_length: int = 512) -> list[jax.Array]:
    ...
    # Padding to a fixed max length keeps static tensor shapes across prompts.
    # Qwen3 is causal and the pad mask is applied, so right padding cannot
    # change the real tokens' states.
    ...
    if self.offload_encoders:
      # Restore parameters from CPU back to original device shardings using device_put_replicated
      state = jax.tree_util.tree_map(
          lambda x, sharding: device_put_replicated(x, sharding), state, self._text_encoder_shardings
      )
    ...
    # Drop the padded positions; Z-Image conditions on the caption tokens.
    prompt_mask = np.asarray(inputs["attention_mask"]).astype(bool)
    return [jnp.asarray(embeddings[index][prompt_mask[index]], dtype=self.dtype) for index in range(batch_size)]
  ...
  def __call__(
    ...
    latents = jax.random.normal(
        jax.random.key(seed), (batch_size, self.transformer.in_channels, latent_height, latent_width), jnp.float32
    )
    sigmas = self._sigmas(num_inference_steps, shift=self.flow_shift)
    ...
    with self.mesh if self.mesh is not None else nullcontext(), nn_partitioning.axis_rules(self.logical_axis_rules):
      for index in range(num_inference_steps):
        ...
        latents = denoise_step(
            ...
            rest,
            latents,
            jnp.full((batch_size,), 1.0 - sigmas[index], dtype=jnp.float32),
            prompt_embeds,

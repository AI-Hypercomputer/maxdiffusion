...
class LTX2Pipeline:
  ...
  def __init__(
    ...
    self.transformer_spatial_patch_size = (
        getattr(self.transformer.config, "patch_size", 1) if getattr(self, "transformer", None) is not None else 1
    )
  ...
  def _pack_latents(latents: jax.Array, patch_size: int = 1, patch_size_t: int = 1) -> jax.Array:
    ...
    latents = latents.transpose(0, 2, 4, 6, 1, 3, 5, 7).reshape(
  ...
  @staticmethod
  def _create_noised_state(latents: jax.Array, noise_scale: float, generator: Optional[nnx.Rngs] = None):
  ...
  @staticmethod
  @jax.jit(static_argnames=["upsampler_module"])
  def _run_upsampler(upsampler_module, params, latents_in):
    return upsampler_module.apply({"params": params}, latents_in)
  ...
  def __call__(
    ...
    axis_rules_context = (
        ...
        if hasattr(self, "config") and hasattr(self.config, "logical_axis_rules")
    ...
    if getattr(self.config, "run_latent_upsampler", False) and self.latent_upsampler is not None:
      ...
      latents_upsampled = self._run_upsampler(self.latent_upsampler, self.latent_upsampler_params, latents)

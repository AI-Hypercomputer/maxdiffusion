...
from ..stable_diffusion_xl.pipeline_output import FlaxStableDiffusionXLPipelineOutput
...
class FlaxStableDiffusionXLControlNetPipeline(FlaxDiffusionPipeline):
  ...
  def get_jit_generate(self, data_sharding, params_sharding):
    p_generate = jax.jit(
        self._generate,
        in_shardings=(data_sharding, params_sharding, None, None, None, None, None, data_sharding, data_sharding, None),
        out_shardings=(None),
        static_argnums=(2, 3, 4, 5, 6, 9),
    )
    return p_generate
...
@partial(
    jax.pmap,
    in_axes=(None, 0, 0, 0, 0, None, 0, 0, 0, None, 0),
    static_broadcasted_argnums=(0, 5, 9),
)
def _p_generate(
    pipe,
    prompt_ids,

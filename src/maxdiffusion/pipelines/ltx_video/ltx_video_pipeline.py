        ...
        if output_type != "latent":
            if self.vae.decoder.timestep_conditioning:
                ...
                if decode_noise_scale is None:
                    ...
                    decode_noise_scale = [
                        decode_noise_scale] * latents.shape[0]

                decode_timestep = torch.tensor(
                    decode_timestep).to(latents.device)
...
def run_inference(
    transformer_state, transformer, config, mesh, latents, fractional_cords, prompt_embeds, timestep, num_inference_steps, scheduler, segment_ids, encoder_attention_segment_ids, scheduler_state, do_classifier_free_guidance, num_conds, guidance_scale
):
    ...
    for step in range(num_inference_steps):
        ...
        noise_pred, transformer_state = transformer_forward_pass(
            latent_model_input, transformer_state, timestep/1000, transformer, fractional_cords, prompt_embeds, segment_ids, encoder_attention_segment_ids)
        ...
        if do_classifier_free_guidance:
            ...
            noise_pred = noise_pred_uncond + guidance_scale[step] * (

...
class LTXVideoPipeline:
  ...
  def load_transformer(cls, config):
    ...
    weights_init_fn = functools.partial(
        transformer.init_weights, in_channels, model_config["caption_channels"], eval_only=True
    )
  ...
  def load_text_encoder(cls, ckpt_path):
    text_encoder = T5EncoderModel.from_pretrained(ckpt_path, subfolder="text_encoder")
    return text_encoder
  ...
  def __call__(
    ...
    latents = self.prepare_latents(
        ...
        timestep=1.0,  # set to 1.0 for now, dummy value

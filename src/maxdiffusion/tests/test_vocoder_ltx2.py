...
class PytorchResBlock(nn.Module):
    def __init__(
        ...
        self.convs1 = nn.ModuleList([
            nn.Conv1d(channels, channels, kernel_size, stride=stride, dilation=d, padding="same")
            for d in dilations
        ])
        self.convs2 = nn.ModuleList([
            nn.Conv1d(channels, channels, kernel_size, stride=stride, dilation=1, padding="same")
            for _ in dilations
        ])
    ...
    def __init__(
        ...
        for i, (stride, kernel_size) in enumerate(zip(upsample_factors, upsample_kernel_sizes)):
            ...
            for k_size, dilations in zip(resnet_kernel_sizes, resnet_dilations):
                self.resnets.append(
                    PytorchResBlock(output_channels, k_size, dilations=dilations, leaky_relu_negative_slope=leaky_relu_negative_slope)
                )
...
class LTX2VocoderTest(unittest.TestCase):
    ...
    def test_resblock_parity(self):
        ...
        jax_res = ResBlock(
            channels=ch, kernel_size=k, dilations=dilations, rngs=self.rng
        )

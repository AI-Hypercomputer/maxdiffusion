...
class WanTransformerTest(unittest.TestCase):
  ...
  def test_wan_self_attention_is_self_attention_dispatch(self):
    """Verifies that FlaxWanAttention with is_self_attention=True dispatches fused RMSNorm+RoPE and matches reference."""
    ...
    with self.mesh, nn_partitioning.axis_rules(self.config.logical_axis_rules):
      ...
      # Exercise both WanTransformerBlock's call (encoder_hidden_states=None)
      # and the explicit identity call (encoder_hidden_states=hidden_states).
      with mock.patch(
          "maxdiffusion.models.attention_flax.fused_rmsnorm_rope",
          wraps=fused_rmsnorm_rope,
      ) as spy_fused:
        out_none = attn(
        ...
        self.assertEqual(spy_fused.call_count, 2)
      self.assertEqual(out.shape, (batch_size, seq_len, query_dim))
      np.testing.assert_array_equal(np.asarray(out_none), np.asarray(out))

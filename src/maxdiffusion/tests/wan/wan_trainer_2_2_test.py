...
class WanTrainer22Test(unittest.TestCase):
  ...
  def test_real_train_step_2_2_execution(self):
    ...
    for _ in range(num_steps):
      ...
      is_high = jnp.bool_(jax.random.uniform(cond_rng) >= config.boundary_ratio)
      state_low, state_high, noise_scheduler_state, metrics, step_rng = jitted_train_step(
          state_low, state_high, data, step_rng, noise_scheduler_state, is_high
      )
  ...
  def test_checkpoint_resume_equivalence(self):
    ...
    for step in range(6):
      ...
      step_rng, cond_rng = jax.random.split(step_rng, num=2)
      is_high = jnp.bool_(jax.random.uniform(cond_rng) >= config.boundary_ratio)
      s_low_a, s_high_a, sched_state_a, _, _ = train_step_fn(s_low_a, s_high_a, data, step_rng, sched_state_a, is_high)
    ...
    for step in range(3):
      ...
      step_rng, cond_rng = jax.random.split(step_rng, num=2)
      is_high = jnp.bool_(jax.random.uniform(cond_rng) >= config.boundary_ratio)
      s_low_b, s_high_b, sched_state_b, _, _ = train_step_fn(s_low_b, s_high_b, data, step_rng, sched_state_b, is_high)
    ...
    for step in range(3, 6):
      ...
      s_low_b_resumed, s_high_b_resumed, sched_state_b, _, _ = train_step_fn(
          s_low_b_resumed, s_high_b_resumed, data, step_rng, sched_state_b, is_high
      )
  ...
  def test_eval_batch_slicing(self):
    """Verify eval_step_2_2 slices loaded batch to global_batch_size_to_train_on."""
    ...
    mock_config.weights_dtype = jnp.float32
    mock_config.disable_training_weights = True
    # Simulate loaded batch of 4, but batch size to train/eval on of 2
    mock_config.global_batch_size_to_train_on = 2
  ...
  def test_compute_unshifted_boundary(self):
    """Verify compute_unshifted_boundary inverts time shift exactly to match shifted boundary_ratio."""
    from maxdiffusion.trainers.wan_trainer_2_2 import compute_unshifted_boundary

    # Identity when shift is 1.0
    self.assertAlmostEqual(compute_unshifted_boundary(0.875, 1.0), 0.875)

    # With shift 12.0 and boundary_ratio 0.875
    # u_boundary = 0.875 / (12.0 - 11.0 * 0.875) = 0.875 / 2.375 ≈ 0.368421
    u_b = compute_unshifted_boundary(0.875, 12.0)
    self.assertAlmostEqual(u_b, 0.875 / 2.375, places=6)

    # Applying forward shift to u_boundary gives back exactly boundary_ratio
    t_shifted = (u_b * 12.0) / (1.0 + 11.0 * u_b)
    self.assertAlmostEqual(t_shifted, 0.875, places=6)

  def test_train_step_expert_timesteps_match_inference_domain(self):
    """Verify high-noise and low-noise experts sample within their respective inference domains."""
    from maxdiffusion.trainers.wan_trainer_2_2 import compute_unshifted_boundary

    shift = 12.0
    boundary_ratio = 0.875
    num_train_timesteps = 1000
    u_boundary = compute_unshifted_boundary(boundary_ratio, shift)

    # Test high-noise expert domain
    rng = jax.random.key(0)
    u_high = jax.random.uniform(rng, (1000,), minval=u_boundary, maxval=1.0)
    t_high = (u_high * shift) / (1.0 + (shift - 1.0) * u_high) * num_train_timesteps

    self.assertTrue(bool(jnp.all(t_high >= boundary_ratio * num_train_timesteps - 1e-4)))
    self.assertTrue(bool(jnp.all(t_high <= num_train_timesteps + 1e-4)))

    # Test low-noise expert domain
    u_low = jax.random.uniform(rng, (1000,), minval=0.0, maxval=u_boundary)
    t_low = (u_low * shift) / (1.0 + (shift - 1.0) * u_low) * num_train_timesteps

    self.assertTrue(bool(jnp.all(t_low >= 0.0 - 1e-4)))
    self.assertTrue(bool(jnp.all(t_low <= boundary_ratio * num_train_timesteps + 1e-4)))

  def test_wan_trainer_2_2_create_scheduler_flow_shift(self):
    """Verify WanTrainer2_2 create_scheduler respects config.flow_shift."""
    from maxdiffusion.trainers.wan_trainer_2_2 import compute_unshifted_boundary

    config = pyconfig.config
    trainer = WanTrainer2_2(config)
    scheduler, state = trainer.create_scheduler()

    self.assertEqual(scheduler.config.shift, 12.0)
    u_boundary = compute_unshifted_boundary(config.boundary_ratio, scheduler.config.shift)
    expected_u_boundary = config.boundary_ratio / (12.0 - 11.0 * config.boundary_ratio)
    self.assertAlmostEqual(u_boundary, expected_u_boundary, places=5)

    # Also verify custom shift of 5.0 via mock config
    mock_config = MagicMock()
    mock_config.flow_shift = 5.0
    mock_config.boundary_ratio = 0.875
    mock_config.train_text_encoder = False
    mock_config.checkpoint_dir = "/tmp/test"
    trainer_5 = WanTrainer2_2(mock_config)
    scheduler_5, _ = trainer_5.create_scheduler()
    self.assertEqual(scheduler_5.config.shift, 5.0)
    u_boundary_5 = compute_unshifted_boundary(0.875, 5.0)
    self.assertAlmostEqual(u_boundary_5, 0.875 / (5.0 - 4.0 * 0.875), places=5)

  def test_metrics_host_filtering_eliminates_d2h_sync(self):
    """Verify metric filtering handles lazy device arrays on host without calling np.isnan."""
    lazy_scalar_metrics = {
        "learning/loss": jnp.array(1.5),
        "learning/loss_low": jnp.array(jnp.nan),
        "learning/loss_high": jnp.array(1.5),
        "learning/loss_low_fine": jnp.array(jnp.nan),
        "learning/max_grad_norm_low": jnp.array(jnp.nan),
        "learning/max_grad_norm_high": jnp.array(0.25),
        "learning/max_abs_grad_low": jnp.array(jnp.nan),
        "learning/max_abs_grad_high": jnp.array(0.1),
    }

    # When is_high_noise is True:
    is_high_noise = True
    train_metric = {"scalar": dict(lazy_scalar_metrics)}
    if is_high_noise:
      train_metric["scalar"] = {k: v for k, v in train_metric["scalar"].items() if "_low" not in k}
      train_metric["scalar"]["learning/current_learning_rate_high"] = 0.001
    else:
      train_metric["scalar"] = {k: v for k, v in train_metric["scalar"].items() if "_high" not in k}
      train_metric["scalar"]["learning/current_learning_rate_low"] = 0.0005

    self.assertIn("learning/loss", train_metric["scalar"])
    self.assertIn("learning/loss_high", train_metric["scalar"])
    self.assertIn("learning/max_grad_norm_high", train_metric["scalar"])
    self.assertIn("learning/current_learning_rate_high", train_metric["scalar"])
    self.assertNotIn("learning/loss_low", train_metric["scalar"])
    self.assertNotIn("learning/loss_low_fine", train_metric["scalar"])
    self.assertNotIn("learning/max_grad_norm_low", train_metric["scalar"])
    self.assertNotIn("learning/current_learning_rate_low", train_metric["scalar"])

    # When is_high_noise is False:
    is_high_noise = False
    train_metric = {"scalar": dict(lazy_scalar_metrics)}
    if is_high_noise:
      train_metric["scalar"] = {k: v for k, v in train_metric["scalar"].items() if "_low" not in k}
    ...
    self.assertIn("learning/loss", train_metric["scalar"])
    self.assertIn("learning/loss_low", train_metric["scalar"])
    self.assertIn("learning/current_learning_rate_low", train_metric["scalar"])
    self.assertNotIn("learning/loss_high", train_metric["scalar"])
    self.assertNotIn("learning/max_grad_norm_high", train_metric["scalar"])
    self.assertNotIn("learning/current_learning_rate_high", train_metric["scalar"])

...
class LTX2PipelineTest(unittest.TestCase):
  ...
  def setUp(self):
    ...
    def tokenizer_side_effect(*args, **kwargs):
      if kwargs.get("return_tensors") == "pt":
        return MagicMock(
            input_ids=torch.zeros((1, 10), dtype=torch.long), attention_mask=torch.ones((1, 10), dtype=torch.long)
        )
      else:
        return MagicMock(input_ids=np.zeros((1, 10), dtype=np.int32), attention_mask=np.ones((1, 10), dtype=np.int32))
  ...
  def test_load_transformer(self, mock_load_config, mock_load_weights):
    ...
    pipeline = LTX2Pipeline.load_transformer(
        devices_array=jnp.array(jax.devices()), mesh=self.mesh, rngs=rngs, config=config, subfolder="transformer"
    )
  ...
  def test_check_inputs(self):
    ...
    with self.assertRaisesRegex(ValueError, "Must provide `negative_prompt_attention_mask`"):
      pipeline.check_inputs(
          ...
          negative_prompt_embeds=jnp.zeros((1, 10, 64)),
      )
  ...
  def test_audio_packing_unpacking(self):
    ...
    unpacked = LTX2Pipeline._unpack_audio_latents(
        packed, latent_length=length, num_mel_bins=mel, patch_size=patch_size, patch_size_t=patch_size_t
    )

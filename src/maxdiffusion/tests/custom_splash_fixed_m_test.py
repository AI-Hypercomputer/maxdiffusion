...
class FixedMMetadataSafetyTest(unittest.TestCase):
  ...
  def test_adversarial_centered_keys_softmax_mass_loss(self):
    ...
    If admitted under a loose one-sided bound (e.g. safe_bound ~ 232), negative logits flush
    to zero in FP32 (S - m_base < -126), losing significant softmax probability mass (0.882 vs 1.0,
    a silent ~0.118 error). The tightened two-sided bound (safe_bound = safe_window // 2) rejects
    ...
    self.assertFalse(bool(all_fixed), "Adversarial centered input must not be eligible for fixed-m")
    self.assertTrue(bool(jnp.all(mk_arr[:, 1] == 0.0)), "fixed_ok predicate must be 0.0 across all blocks")

    # Verify that the kernel execution safely falls back to online softmax, preserving
    # the negative logit probability mass near 15/17 (~0.8824) rather than flushing to 1.0.
    ...
    self.assertLess(
        ...
        5e-3,
        f"Fallback online softmax must preserve negative logit probability mass near 15/17 (~0.8824), got {float(out[0, 0, 0]):.6f}",
    )

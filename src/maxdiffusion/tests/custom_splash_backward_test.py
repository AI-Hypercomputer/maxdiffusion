...
class CustomSplashBackwardTest(unittest.TestCase):
  ...
  def test_single_device_bwd_unpadded_non_divisible_seqlens(self):
    ...
    for use_fused, dq_red in [(True, 3), (True, None), (False, None)]:
      ...
      self.assertLess(float(jnp.max(jnp.abs(out_k.astype(jnp.float32) - out_ref.astype(jnp.float32)))), 1e-3)
      self.assertLess(float(jnp.max(jnp.abs(dq_k.astype(jnp.float32) - dq_ref.astype(jnp.float32)))), 1e-2)
      self.assertLess(float(jnp.max(jnp.abs(dk_k.astype(jnp.float32) - dk_ref.astype(jnp.float32)))), 1e-2)
      self.assertLess(float(jnp.max(jnp.abs(dv_k.astype(jnp.float32) - dv_ref.astype(jnp.float32)))), 1e-2)

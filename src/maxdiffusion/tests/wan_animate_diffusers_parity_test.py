...
class WanAnimateDiffusersParityTest(unittest.TestCase):
  ...
  def test_prepare_prev_segment_cond_latents_animate_encodes_full_segment_like_diffusers(
      self,
  ):
    call_lengths = []
    ...
    def fake_encode(video, dtype):
      ...
      return jnp.ones(
          (video.shape[0], latent_t, latent_h, latent_w, self.max_pipeline.vae.z_dim),
          dtype=jnp.float32,
      )
  ...
  def test_prepare_prev_segment_cond_latents_animate_first_segment_encodes_zero_filled_segment_like_diffusers(
      self,
  ):
    call_lengths = []
  ...
  def test_animate_transformer_forward_pass_matches_diffusers_layout(self):
    ...
    class FakeTransformer:
      ...
      def __call__(
        ...
        capture["face_motion_vec"] = face_motion_vec
    ...
    face_motion_vec = self._random_jax_array((1, 5, 8))
    timestep = jnp.array([7], dtype=jnp.int32)
    prompt_embeds = self._random_jax_array((1, 4, 3))
    image_embeds = self._random_jax_array((1, 4, 3))

    with mock.patch(
        "maxdiffusion.pipelines.wan.wan_pipeline_animate.nnx.merge",
        return_value=FakeTransformer(),
    ):
      noise_pred = animate_transformer_forward_pass.__wrapped__(
          ...
          face_motion_vec=face_motion_vec,
          timestep=timestep,
    ...
    np.testing.assert_allclose(
        to_numpy(capture["hidden_states"]),
        to_numpy(expected_hidden),
        atol=0.0,
        rtol=0.0,
    )
    np.testing.assert_allclose(
        to_numpy(capture["pose_hidden_states"]),
        to_numpy(expected_pose),
        atol=0.0,
        rtol=0.0,
    )
    np.testing.assert_allclose(
        to_numpy(capture["face_motion_vec"]),
        to_numpy(face_motion_vec),
        atol=0.0,
        rtol=0.0,
    )
    np.testing.assert_allclose(
        to_numpy(capture["encoder_hidden_states"]),
        to_numpy(prompt_embeds),
        atol=0.0,
        rtol=0.0,
    )
    np.testing.assert_allclose(
        to_numpy(capture["encoder_hidden_states_image"]),
        to_numpy(image_embeds),
        atol=0.0,
        rtol=0.0,
    )
  ...
  def test_single_denoising_step_matches_diffusers_with_cfg(self):
    ...
    max_face_motion = self._random_jax_array((1, 5, 8))
    max_face_motion_uncond = self._random_jax_array((1, 5, 8))
    ...
    hf_face_motion = torch.tensor(to_numpy(max_face_motion))
    hf_face_motion_uncond = torch.tensor(to_numpy(max_face_motion_uncond))
    ...
    with mock.patch(
      ...
      max_noise_cond = animate_transformer_forward_pass.__wrapped__(
          ...
          face_motion_vec=max_face_motion,
      ...
      max_noise_uncond = animate_transformer_forward_pass.__wrapped__(
          ...
          reference_latents=max_reference,
          pose_latents=max_pose,
          face_motion_vec=max_face_motion_uncond,
          timestep=max_timestep,
    ...
    hf_noise_cond = fake_transformer(
        ...
        face_motion_vec=hf_face_motion,
        return_dict=False,
    )[0]
    hf_noise_uncond = fake_transformer(
        ...
        encoder_hidden_states_image=hf_image,
        pose_hidden_states=hf_pose,
        face_motion_vec=hf_face_motion_uncond,
        return_dict=False,
    )[0]
  ...
  def test_call_runs_multisegment_pipeline_and_trims_output(self):
    ...
    def fake_encode_face_motion(
      ...
      del graphdef, sharded_state, rest_of_state, motion_encode_batch_size
      return jnp.ones((batch_size, 5, 8), dtype=jnp.float32)
    ...
    def fake_denoise(
      ...
      denoise_shapes.append((latents.shape, pose_latents.shape, face_motion_vec.shape, timestep.shape))
    ...
    with (
        mock.patch(
            "maxdiffusion.pipelines.wan.wan_pipeline_animate.nnx.split",
            return_value=(None, None, None),
        ),
        mock.patch(
            "maxdiffusion.pipelines.wan.wan_pipeline_animate.animate_encode_face_motion",
            side_effect=fake_encode_face_motion,
        ),
        mock.patch(
            "maxdiffusion.pipelines.wan.wan_pipeline_animate.animate_transformer_forward_pass",
            side_effect=fake_denoise,
        ),
        mock.patch.object(self.max_pipeline, "_decode_segment_to_pixels", side_effect=fake_decode),
    ):
    ...
    self.assertEqual(denoise_shapes[0], ((1, 4, 2, 2, 2), (1, 3, 2, 2, 2), (1, 5, 8), (1,)))

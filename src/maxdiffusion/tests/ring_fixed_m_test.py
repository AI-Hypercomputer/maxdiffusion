...
class RingFixedMTest(unittest.TestCase):
  ...
  def _shard_max_sq_norms(self, x):
    """(heads, ring_size) max squared row norm of each rank's shard."""
    xf = x.astype(jnp.float32)
    sq = (xf * xf).sum(-1).reshape(self.num_heads, _RING_SIZE, self.shard_len)
  ...
  def _gate_global(self, q_in, k_in):
    ...
    _, global_bound = custom_splash.get_fixed_m_constants(self.shard_len * _RING_SIZE)
    qn_sq = self._shard_max_sq_norms(q_in).max(axis=1)
    kn_sq = self._shard_max_sq_norms(k_in).max(axis=1)
    return qn_sq * kn_sq <= global_bound**2
  ...
  def _global_gate(self, q, k):
    return self._gate_global(*self._scaled_inputs(q, k))
  ...
  def test_fixed_m_all_eligible_matches_reference(self):
    q, k, v = self._random_qkv()
    self.assertTrue(bool(jnp.all(self._global_gate(q, k))))  # accumulate merge
  ...
  def test_sink_head_falls_back_everywhere(self):
    ...
    global_gate = self._global_gate(q, k)
    self.assertFalse(bool(global_gate[0]))  # forces the per-hop LSE path
    self.assertTrue(bool(jnp.all(global_gate[1:])))
    gate = self._gate(q, k)
    self.assertFalse(bool(jnp.any(gate[0])))  # head 0 online on every (rank, shard)
    self.assertTrue(bool(jnp.all(gate[1:])))  # other heads fixed on every hop
    self.assertLess(self._run_and_compare(q, k, v, use_fixed_m=True), 2e-2)
  ...
  def test_fixed_m_accumulate_ragged_tail(self):
    ...
    q, k, v = self._random_qkv()
    self.assertTrue(bool(jnp.all(self._global_gate(q, k))))
    self.assertTrue(bool(jnp.all(self._gate(q, k))))
    self.assertLess(self._run_and_compare(q, k, v, use_fixed_m=True), 2e-2)
  ...
  def test_declared_unsquared_norms_match_squared(self):
    """The two declared norm representations must produce the same output.

    The norm representation is declared via `fixed_m_norms_squared`; it can't
    be inferred from magnitude. Both spellings of the same inputs must produce
    the same output.
    """
...
class RingFixedMContractTest(unittest.TestCase):
  ...
  def test_per_head_norms_with_per_q_block_are_rejected(self):
    ...
    Both eligibility gates compute `qn * mk[:, None]`, so a (heads,) array
    does not raise under per_q_block=True -- it broadcasts to (heads, heads),
    pairing head j's query norm with head h's key norm. A sink head inherits a
    small bound from an unrelated head, is wrongly marked fixed-eligible, and
    the kernel then evaluates exp2(large_logit - small_m), which overflows to
    inf.
    """
  ...
  def test_fp16_is_rejected_by_dtype_safety(self):
    ...
    2**recenter with recenter = C(4096) = 107, far beyond float16's ceiling.
    """
...
class RingRawKeyBoundUnsoundTest(unittest.TestCase):
  """Why virtual K-centering on the ring path has to be opt-in.

  Centering and the Cauchy-Schwarz bound are a matched pair. If a kernel
  exponentiates centered logits `q . (k_j - k_mean)` while the caller's
  eligibility bound was built from the raw, uncentered `k`, the bound caps the
  wrong quantity: `||k||` does not bound `||k - k_mean||`, so a tile can clear
  the gate while the centered logit it is supposed to cap overflows fp32.

  That is why `make_custom_ring_attention` centers only when the caller passes
  `k_mean` -- a kernel that centered on its own would silently invalidate every
  caller that had not also been taught to centre its norms. Callers that want
  centering supply `k_mean` and centered norms together; callers that do not
  keep the uncentered path, which is sound against the two-sided `floor(W/2)`
  bound.
  ...
  def test_raw_bound_admits_a_tile_the_centered_bound_rejects(self):
    ...
    _, safe_bound = custom_splash.get_fixed_m_constants(self.total_kv)
  ...
  def test_centering_restores_a_sound_bound(self):
    ...
    query, keys = self._adversarial_keys()
    recenter, safe_bound = custom_splash.get_fixed_m_constants(self.total_kv)

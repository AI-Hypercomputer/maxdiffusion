...
Context
-------
Production pads Q to `block_q` with `_pad_data_for_flash`. K/V are passed
unpadded when the KV length is 8-aligned (C3a, relied on by `kv_pad_size = 1`
in `_ulysses_attention` and `_ulysses_ring_custom_attention`); otherwise they
are padded to `block_kv`. These tests gate that K/V behaviour and the
still-proposed Q unpadding (C3b). Static analysis says the padding is
unnecessary:
...
class CustomSplashUnpaddedInputTest(unittest.TestCase):
  ...
  def _inputs(self):
    ...
    q = jax.random.normal(
        ...
        (self.heads, self.q_len, self.head_dim),
        jnp.bfloat16,
    )
    k = jax.random.normal(
        jax.random.PRNGKey(12),
        (self.heads, self.kv_len, self.head_dim),
        jnp.bfloat16,
    )
    v = jax.random.normal(
  ...
  def _metadata(self, q_in, k_in):
    ...
    recenter, safe_bound = custom_splash.get_fixed_m_constants(self.kv_len)
    ...
    mk = jnp.stack(
        [jnp.ceil(bound) - recenter, (bound <= safe_bound).astype(jnp.float32)],
        axis=0,
    )
    return mk, k_mean
  ...
  def _compare(
    ...
    self.assertTrue(
        bool(jnp.all(jnp.isfinite(candidate))),
        "unpadded run produced non-finite values",
    )
    self.assertTrue(

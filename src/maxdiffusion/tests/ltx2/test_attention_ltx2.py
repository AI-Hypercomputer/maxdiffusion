...
class PytorchLTX2Attention(torch.nn.Module):
  ...
  def forward(self, x, context=None, q_rope=None, k_rope=None, mask=None):
    ...
    out = torch.nn.functional.scaled_dot_product_attention(q_h, k_h, v_h, attn_mask=mask, dropout_p=0.0)
...
class LTX2AttentionTest(unittest.TestCase):
  ...
  def _init_and_sync_models(self, dtype=jnp.bfloat16):
    ...
    def to_jax_dtype(arr):
      return jnp.array(arr).astype(dtype)
  ...
  def test_parity_bf16_strict(self):
    ...
    np.testing.assert_allclose(pt_res, jax_res, atol=2e-2, rtol=1e-2, err_msg="BF16 Parity Failed")
  ...
  def test_cross_attn_rope_integration(self):
    ...
    with torch.no_grad():
      pt_out, _ = pt_model(
          torch.from_numpy(np_x), context=torch.from_numpy(np_ctx), q_rope=(q_cos_pt, q_sin_pt), k_rope=(k_cos_pt, k_sin_pt)
      )
    ...
    jax_out = jax_model(
        jnp.array(np_x), encoder_hidden_states=jnp.array(np_ctx), rotary_emb=jax_q_rope, k_rotary_emb=jax_k_rope
    )
  ...
  def test_attention_mask_parity(self):
    ...
    with mesh:
      jax_out = jax_model(jnp.array(np_x), attention_mask=jax_mask_multiplicative)

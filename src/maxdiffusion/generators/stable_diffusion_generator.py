import functools
import time
import numpy as np
import jax
import jax.numpy as jnp
from jax.sharding import PartitionSpec as P
from flax.linen import partitioning as nn_partitioning
from maxdiffusion.generators.base_stable_diffusion_generator import (
    BaseStableDiffusionGenerator
)
from maxdiffusion import (
    FlaxDDIMScheduler,
    max_logging
)
from maxdiffusion.image_processor import VaeImageProcessor

from maxdiffusion.maxdiffusion_utils import rescale_noise_cfg

from maxdiffusion.checkpointing.base_stable_diffusion_checkpointer import (
    STABLE_DIFFUSION_CHECKPOINT
)

class StableDiffusionGenerator(BaseStableDiffusionGenerator):
    def __init__(self, config):
        BaseStableDiffusionGenerator.__init__(self, config, STABLE_DIFFUSION_CHECKPOINT)

    def post_create_states_and_shard(self):
        return super().post_create_states_and_shard()
    
    def pre_generating_steps(self):
        return super().pre_generating_steps()

    def tokenize(self, prompt, tokenizer):
        """Tokenizes prompt."""
        return tokenizer(
            prompt,
            padding="max_length",
            max_length=tokenizer.model_max_length,
            truncation=True,
            return_tensors="np"
        ).input_ids

    def create_scheduler(self):
        scheduler, scheduler_state = FlaxDDIMScheduler.from_pretrained(
            self.config.pretrained_model_name_or_path, revision=self.config.revision, subfolder="scheduler", dtype=jnp.float32
        )
        self.pipeline.scheduler = scheduler
        self.params["scheduler"] = scheduler_state
    
    def get_inputs(self):
        rng = self.rng
        batch_size = self.total_train_batch_size
        params = self.params
        states = self.train_states
        config = self.config
        pipeline = self.pipeline
        mesh = self.mesh

        data_sharding = jax.sharding.NamedSharding(mesh,P(*config.data_sharding))

        vae_scale_factor = 2 ** (len(pipeline.vae.config.block_out_channels) - 1)
        prompt_ids = [config.prompt] * batch_size
        prompt_ids = self.tokenize(prompt_ids, pipeline.tokenizer)
        negative_prompt_ids = [config.negative_prompt] * batch_size
        negative_prompt_ids = self.tokenize(negative_prompt_ids, pipeline.tokenizer)
        guidance_scale = config.guidance_scale
        guidance_rescale = config.guidance_rescale
        num_inference_steps = config.num_inference_steps

        prompt_embeds = pipeline.text_encoder(prompt_ids, params=states["text_encoder_state"].params)[0]
        negative_prompt_embeds = pipeline.text_encoder(negative_prompt_ids, params=states["text_encoder_state"].params)[0]
        context = jnp.concatenate([negative_prompt_embeds, prompt_embeds])
        guidance_scale = jnp.array([guidance_scale], dtype=jnp.float32)
        guidance_rescale = jnp.array([guidance_rescale], dtype=jnp.float32)

        batch_size = prompt_ids.shape[0]
        latents_shape = (
            batch_size,
            pipeline.unet.config.in_channels,
            config.resolution // vae_scale_factor,
            config.resolution // vae_scale_factor,
        )
        latents = jax.random.normal(rng, shape=latents_shape, dtype=jnp.float32)

        scheduler_state = pipeline.scheduler.set_timesteps(
            params["scheduler"],
            num_inference_steps=num_inference_steps,
            shape=latents.shape
        )
        latents = latents * params["scheduler"].init_noise_sigma

        latents = jax.device_put(latents, data_sharding)
        context = jax.device_put(context, data_sharding)

        return latents, context, guidance_scale, guidance_rescale, scheduler_state

    def vae_decode(self, latents, state, pipeline):
        latents = 1 / pipeline.vae.config.scaling_factor * latents
        image = pipeline.vae.apply(
            {"params" : state.params},
            latents,
            method=pipeline.vae.decode
        ).sample
        image = (image / 2 + 0.5).clip(0, 1).transpose(0, 2, 3, 1)
        return image

    def generation_loop(self):
        states = self.train_states
        return np.array(self.p_run_inference(states["unet_state"], states["vae_state"]))
    
    def post_generation_steps(self, numpy_images):
        images = VaeImageProcessor.numpy_to_pil(numpy_images)
        for i, image in enumerate(images):
            image.save(f"image_{i}.png")
        
    def compile_gen_step(self):

        state_shardings = self.state_shardings
        states = self.train_states

        with self.mesh, nn_partitioning.axis_rules(self.config.logical_axis_rules):
            p_run_inference = jax.jit(
                functools.partial(_generate, generator=self),
                in_shardings=(state_shardings["unet_state_shardings"], state_shardings["vae_state_shardings"]),
                out_shardings=None
            )
            max_logging.log("Precompiling...")
            s = time.time()
            self.p_run_inference = p_run_inference.lower(states["unet_state"], states["vae_state"]).compile()
            max_logging.log(f"Compile time: {(time.time() - s)}")


def _generate(unet_state, vae_state, generator : BaseStableDiffusionGenerator):
    pipeline = generator.pipeline
    config = generator.config

    (latents,
    context,
    guidance_scale,
    guidance_rescale,
    scheduler_state) = generator.get_inputs()
    
    loop_body_p = functools.partial(_loop_body,
                                    pipeline=pipeline,
                                    prompt_embeds=context,
                                    guidance_scale=guidance_scale,
                                    guidance_rescale=guidance_rescale)
    
    vae_decode_p = functools.partial(generator.vae_decode, pipeline=pipeline)

    latents, _, _ = jax.lax.fori_loop(0, config.num_inference_steps, loop_body_p, (latents, scheduler_state, unet_state))

    image = vae_decode_p(latents, vae_state)
    return image

def _loop_body(step, args, pipeline, prompt_embeds, guidance_scale, guidance_rescale):
    latents, scheduler_state, state = args
    latents_input = jnp.concatenate([latents] * 2)

    t = jnp.array(scheduler_state.timesteps, dtype=jnp.int32)[step]
    timestep = jnp.broadcast_to(t, latents_input.shape[0])

    latents_input = pipeline.scheduler.scale_model_input(scheduler_state, latents_input, t)
    
    noise_pred = pipeline.unet.apply(
        {"params" : state.params},
        jnp.array(latents_input),
        jnp.array(timestep, dtype=jnp.int32),
        encoder_hidden_states=prompt_embeds
    ).sample

    noise_pred_uncond, noise_prediction_text = jnp.split(noise_pred, 2, axis=0)
    noise_pred = noise_pred_uncond + guidance_scale * (noise_prediction_text - noise_pred_uncond)

    # Based on 3.4. in https://arxiv.org/pdf/2305.08891.pdf
    noise_pred = rescale_noise_cfg(noise_pred, noise_prediction_text, guidance_rescale=guidance_rescale)

    latents, scheduler_state = pipeline.scheduler.step(scheduler_state, noise_pred, t, latents).to_tuple()
    return latents, scheduler_state, state
    
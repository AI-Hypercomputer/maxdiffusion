...
from .unet_2d_blocks_flax import FlaxCrossAttnDownBlock2D, FlaxDownBlock2D, FlaxUNetMidBlock2DCrossAttn
...
class FlaxControlNetModel(nn.Module, FlaxModelMixin, ConfigMixin):
  ...
  def init_weights(self, rng: jax.Array, eval_only=False) -> FrozenDict:
    ...
    return self.init(rngs, sample, timesteps, encoder_hidden_states, controlnet_cond, added_cond_kwargs=added_cond_kwargs)[
        "params"
    ]
  ...
  def setup(self):
    ...
    if self.addition_embed_type == "text_time":
      self.add_time_proj = FlaxTimesteps(
          self.addition_time_embed_dim, flip_sin_to_cos=self.flip_sin_to_cos, freq_shift=self.config.freq_shift
      )

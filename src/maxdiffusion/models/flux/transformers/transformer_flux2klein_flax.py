...
class Flux2KleinTransformer2DModel(nn.Module, FlaxModelMixin, ConfigMixin):
    ...
    def __call__(
        ...
        print(f"DEBUG TRANSFORMER: before double blocks, hidden_states={hidden_states.shape}, encoder_hidden_states={encoder_hidden_states.shape}")
        if return_intermediates:
        ...
        print(f"DEBUG TRANSFORMER: after double blocks, hidden_states={hidden_states.shape}, encoder_hidden_states={encoder_hidden_states.shape}")
        num_txt_tokens = encoder_hidden_states.shape[1]
        hidden_states = jnp.concatenate([encoder_hidden_states, hidden_states], axis=1)
        print(f"DEBUG TRANSFORMER: concatenated single block input={hidden_states.shape}")
        ...
        print(f"DEBUG TRANSFORMER: after single blocks={hidden_states.shape}, num_txt_tokens={num_txt_tokens}")
        hidden_states = hidden_states[:, num_txt_tokens:, ...]
        print(f"DEBUG TRANSFORMER: after slicing txt tokens={hidden_states.shape}")
        hidden_states = self.norm_out(hidden_states, temb)
        output = self.proj_out(hidden_states)
        print(f"DEBUG TRANSFORMER: output={output.shape}")

...
class Flux2KleinTransformer2DModel(nn.Module, FlaxModelMixin, ConfigMixin):
  ...
  def __call__(
    ...
    print(
        f"DEBUG TRANSFORMER: before double blocks, hidden_states={hidden_states.shape}, encoder_hidden_states={encoder_hidden_states.shape}"
    )
    if return_intermediates:
    ...
    print(
        f"DEBUG TRANSFORMER: after double blocks, hidden_states={hidden_states.shape}, encoder_hidden_states={encoder_hidden_states.shape}"
    )

...
from maxdiffusion.models.ltx_video.autoencoders.conv_nd_factory import make_conv_nd
...
class AutoencoderKLWrapper(ModelMixin, ConfigMixin):
  ...
  def enable_z_tiling(self, z_sample_size: int = 8):
    ...
    assert z_sample_size % 8 == 0 or z_sample_size == 1, f"z_sample_size must be a multiple of 8 or 1. Got {z_sample_size}."
  ...
  def encode(self, z: torch.FloatTensor, return_dict: bool = True) -> Union[DecoderOutput, torch.FloatTensor]:
    if self.use_z_tiling and z.shape[2] > self.z_sample_size > 1:
      ...
      moments_tiles = [
          (self._hw_tiled_encode(z_tile, return_dict) if self.use_hw_tiling else self._encode(z_tile)) for z_tile in tiles
      ]
  ...
  def decode(
    ...
    if self.use_z_tiling and z.shape[2] > self.z_sample_size > 1:
      reduction_factor = int(
          self.encoder.patch_size_t * 2 ** (len(self.encoder.down_blocks) - 1 - math.sqrt(self.encoder.patch_size))
      )
      ...
      decoded = (
          self._hw_tiled_decode(
              z, target_shape
          )  # z(1, 128, 1, 6, 8), need to be torch.Size([]) target (1, 3, 8, 192, 256), should be type Tensor
          if self.use_hw_tiling
          else self._decode(
              z, target_shape=target_shape, timestep=timestep
          )  # Tensor(<class 'jaxlib._jax.ArrayImpl'> 0.05) size torch.Size([])
      )

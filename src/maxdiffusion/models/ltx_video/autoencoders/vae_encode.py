...
def vae_encode(
  ...
  if split_size > 1:
    ...
    dist = vae.encode(media_items).latent_dist
    latents = dist.sample()
...
def vae_decode(  # this function needs latents to be in tensor form
    latents: Tensor,
    vae: AutoencoderKL,
    is_video: bool = True,
    split_size: int = 1,
  ...
  if split_size > 1:
    ...
    image_batch = [
        _run_decoder(latent_batch, vae, is_video, vae_per_channel_normalize, timestep)
...
def latent_to_pixel_coords(latent_coords: Tensor, vae: AutoencoderKL, causal_fix: bool = False) -> Tensor:
  """
  ...
  pixel_coords = latent_to_pixel_coords_from_factors(latent_coords, scale_factors, causal_fix)
  return pixel_coords


def latent_to_pixel_coords_from_factors(latent_coords: Tensor, scale_factors: Tuple, causal_fix: bool = False) -> Tensor:
  pixel_coords = latent_coords * torch.tensor(scale_factors, device=latent_coords.device)[None, :, None]
  if causal_fix:

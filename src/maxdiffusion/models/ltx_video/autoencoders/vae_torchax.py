...
class TorchaxCausalVideoAutoencoder(interop.JittableModule):
    ...
    def decode(self, latents: jax.Array, timestep: jax.Array, split_size: int = 1, vae_per_channel_normalize: bool = True, is_video: bool = True) -> jax.Array:
        with default_env():
    ...
    @staticmethod
    def _vae_decoder_inner(model, latents, timestep, is_video: bool = True, split_size: int = 1, vae_per_channel_normalize: bool = False):
        return vae_decode(
            latents=latents,
            vae=model,
            is_video=is_video,

...
from maxdiffusion.models.ltx_video.autoencoders.conv_nd_factory import make_conv_nd, make_linear_nd
from maxdiffusion.models.ltx_video.autoencoders.pixel_norm import PixelNorm
from maxdiffusion.models.ltx_video.autoencoders.pixel_shuffle import PixelShuffleND
from maxdiffusion.models.ltx_video.autoencoders.vae import AutoencoderKLWrapper
from maxdiffusion.models.ltx_video.transformers.attention import Attention
from maxdiffusion.models.ltx_video.utils.diffusers_config_mapping import (
...
class Encoder(nn.Module):
  ...
  def forward(self, sample: torch.FloatTensor) -> torch.FloatTensor:
    ...
    sample = self.conv_norm_out(sample).to(torch.bfloat16)
...
class Decoder(nn.Module):
  ...
  def forward(
    ...
    if self.timestep_conditioning:
      ...
      ada_values = self.last_scale_shift_table[None, ..., None, None, None] + embedded_timestep.reshape(
          batch_size,
          2,
          -1,
...
class ResnetBlock3D(nn.Module):
  ...
  def forward(
    ...
    hidden_states = self.norm1(hidden_states).to(torch.bfloat16)
    if self.timestep_conditioning:
      assert timestep is not None, "should pass timestep with timestep_conditioning=True"
      ada_values = self.scale_shift_table[None, ..., None, None, None] + timestep.reshape(
          batch_size,
          4,
          -1,
          timestep.shape[-3],
    ...
    hidden_states = self.norm2(hidden_states).to(torch.bfloat16)
...
def demo_video_autoencoder_forward_backward():
  ...
  reconstructed_videos = video_autoencoder.decode(latent, target_shape=input_videos.shape, timestep=timestep).sample
  ...
  _ = video_autoencoder.decode(image_latent, target_shape=image_latent.shape, timestep=timestep).sample

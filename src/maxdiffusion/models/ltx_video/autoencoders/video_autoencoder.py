...
class VideoAutoencoder(AutoencoderKLWrapper):
  ...
  def config(self):
    return SimpleNamespace(
        ...
        block_out_channels=[
            self.encoder.down_blocks[i].res_blocks[-1].conv1.out_channels for i in range(len(self.encoder.down_blocks))
        ],
        scaling_factor=1.0,
...
class Encoder(nn.Module):
  ...
  def forward(self, sample: torch.FloatTensor, return_features=False) -> torch.FloatTensor:

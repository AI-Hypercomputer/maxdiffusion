...
class Transformer3DModel(nn.Module):
    ...
    def create_skip_layer_mask(
        ...
        if skip_block_list is None or len(skip_block_list) == 0:
            return None
        mask = jnp.ones(
            (self.num_layers, batch_size * num_conds), dtype=self.dtype
        )

        for block_idx in skip_block_list:
            mask = mask.at[block_idx, ptb_index::num_conds].set(0)
    ...
    def __call__(
        ...
        scale_shift_values = (
            self.scale_shift_table[jnp.newaxis, jnp.newaxis, :, :] + embedded_timestep[:, :, jnp.newaxis]
        )

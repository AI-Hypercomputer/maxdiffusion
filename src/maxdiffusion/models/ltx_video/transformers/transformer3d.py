...
class Transformer3DModel(nn.Module):
    ...
    def setup(self):
        ...
        self.scale_shift_table = self.param(
            ...
            nn.with_logical_partitioning(
                scale_shift_table_init, ("ada", "embed")),
        )
        self.norm_out = nn.LayerNorm(
            epsilon=1e-6, use_scale=False, use_bias=False)
    ...
    def init_weights(self, key, batch_size, text_tokens, num_tokens, features, eval_only=True):
        ...
        prompt_embeds = jax.random.normal(
            split_key, shape=prompt_embeds_shape, dtype=latents_dtype)
        key, split_key = jax.random.split(key)
        fractional_coords = jax.random.normal(
            split_key, shape=fractional_cords_shape, dtype=fractional_coords_dtype)
        key, split_key = jax.random.split(key)
        latents = jax.random.normal(
            split_key, shape=latents_shape, dtype=prompt_embeds_dtype)
        key, split_key = jax.random.split(key)
        noise_cond = jax.random.normal(
            split_key, shape=noise_cond_shape, dtype=noise_cond_dtype)
    ...
    def __call__(
        ...
        scale_shift_values = (
            self.scale_shift_table[jnp.newaxis, jnp.newaxis,
                                   :, :] + embedded_timestep[:, :, jnp.newaxis]
        )
        scale_shift_values = nn.with_logical_constraint(
            scale_shift_values, ("activation_batch", "activation_length",
...
class FreqsCisPrecomputer(nn.Module):
    ...
    def __call__(self, indices_grid: jax.Array) -> Tuple[jax.Array, jax.Array]:
        ...
        freqs = (indices * (jnp.expand_dims(fractional_positions,
                 axis=-1) * 2 - 1)).swapaxes(-1, -2)

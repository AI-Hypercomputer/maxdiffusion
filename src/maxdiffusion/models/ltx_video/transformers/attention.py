...
class BasicTransformerBlock(nn.Module):
    ...
    def setup(self):
        ...
        assert self.adaptive_norm in [
            "single_scale_shift", "single_scale", "none"]
    ...
    def __call__(
        ...
        hidden_states = nn.with_logical_constraint(
            hidden_states, ("activation_batch",
                            "activation_norm_length", "activation_embed")
        )
        hidden_states = checkpoint_name(
            hidden_states, "basic_transformer_block hidden_states")
        ...
        norm_hidden_states = nn.with_logical_constraint(
            norm_hidden_states, ("activation_batch",
                                 "activation_norm_length", "activation_embed")
        )
        ...
        if self.adaptive_norm in ["single_scale_shift", "single_scale"]:
            ...
            ada_values = nn.with_logical_constraint(
                ada_values, ("activation_batch", "activation_norm_length",
                             "activation_ada", "activation_embed")
            )
            ...
            if self.adaptive_norm == "single_scale_shift":
                ...
                scale_msa, gate_msa, scale_mlp, gate_mlp = (
                    jnp.squeeze(arr, axis=2) for arr in jnp.split(ada_values, 4, axis=2)
                )
            ...
            raise ValueError(
                f"Unknown adaptive norm type: {self.adaptive_norm}")
        ...
        if self.attn2 is not None:
            attn_input = self.attn2_norm(
                hidden_states) if self.adaptive_norm == "none" else hidden_states
            attn_input = nn.with_logical_constraint(
                attn_input, ("activation_batch",
                             "activation_norm_length", "activation_embed")
            )
        ...
        ff_output = nn.with_logical_constraint(
            ff_output, ("activation_batch",
                        "activation_norm_length", "activation_embed")
        )
...
class Attention(nn.Module):
    ...
    def setup(self):
        ...
        if self.attention_op is not None:
            ...
            _tpu_available = any(
                device.platform == "tpu" for device in jax.devices())
            self.attention = AttentionOp() if _tpu_available else ExplicitAttention()
            if not _tpu_available:
                print(
                    "Warning: Running with explicit attention since tpu is not available.")
    ...
    def __call__(
        ...
        **cross_attention_kwargs,
        ...
        assert cross_attention_kwargs.get(
            "scale", None) is None, "Not supported"

        input_axis_names = ("activation_batch",
                            "activation_length", "activation_embed")
        hidden_states = nn.with_logical_constraint(
            hidden_states, input_axis_names)
        if encoder_hidden_states is not None:
            encoder_hidden_states = nn.with_logical_constraint(
                encoder_hidden_states, input_axis_names)
        ...
        if input_ndim == 4:
            ...
            hidden_states = jnp.reshape(
                hidden_states, (batch_size, channel, height * width))
        ...
        query = nn.with_logical_constraint(
            query, ("activation_kv_batch", "activation_kv_heads",
                    "activation_length", "activation_kv_head_dim")
        )
        ...
        key = nn.with_logical_constraint(
            key, ("activation_kv_batch", "activation_kv_heads",
                  "activation_length", "activation_kv_head_dim")
        )
        ...
        value = nn.with_logical_constraint(
            value, ("activation_kv_batch", "activation_kv_heads",
                    "activation_length", "activation_kv_head_dim")
        )
        ...
        if kv_attention_segment_ids is not None and q_segment_ids is None:
            q_segment_ids = jnp.ones(
                (batch_size, query.shape[2]), dtype=jnp.float32)

        hidden_states_a = self.attention(
            query, key, value, q_segment_ids, kv_attention_segment_ids, sharding_mesh, self.dtype
        )

        hidden_states_a: jax.Array = nn.with_logical_constraint(
            hidden_states_a, ("activation_kv_batch", "activation_heads",
                              "activation_length", "activation_kv")
        )

        hidden_states_a = jnp.reshape(jnp.swapaxes(
            hidden_states_a, 1, 2), (batch_size, -1, self.heads * head_dim))
        if skip_layer_mask is not None and skip_layer_strategy == SkipLayerStrategy.AttentionSkip:
            hidden_states = hidden_states_a * skip_layer_mask + \
                hidden_states * (1.0 - skip_layer_mask)
        else:
            hidden_states = hidden_states_a

        hidden_states = self.to_out[0](hidden_states)
        hidden_states = self.to_out[1](
            hidden_states, deterministic=deterministic)  # Dropout

        if input_ndim == 4:
            hidden_states = jnp.reshape(jnp.swapaxes(
                hidden_states, -1, -2), (batch_size, channel, height, width))
            if skip_layer_mask is not None and skip_layer_strategy == SkipLayerStrategy.Residual:
                skip_layer_mask = jnp.reshape(
                    skip_layer_mask, (batch_size, 1, 1, 1))
    ...
    def prepare_attention_mask(
        ...
        if current_length != target_length:
            ...
            attention_mask = jnp.pad(
                attention_mask, ((0, 0), (0, remaining_length)), constant_values=0.0)
    ...
    def __call__(
        ...
        def partial_flash_attention(q, k, v, q_segment_ids, kv_segment_ids):
            s = (
                ...
                SegmentIds(q_segment_ids.astype(jnp.float32),
                           kv_segment_ids.astype(jnp.float32))
        ...
        if sharding_mesh is not None:
            ...
            if q_segment_ids is not None and q_segment_ids.ndim != 2:
                raise ValueError(
                    f"Expected mask with 2 dims, got {q_segment_ids.ndim}.")
            ...
            qkv_segment_ids_spec = jax.sharding.PartitionSpec(
                ("data", "fsdp", "fsdp_transpose", "expert"), "sequence")
...
class RMSNorm(nn.Module):
    ...
    def __call__(self, hidden_states: jax.Array) -> jax.Array:
        ...
        if self.elementwise_affine:
            scale = self.param(
                ...
                nn.with_logical_partitioning(
                    self.scale_init, self.kernel_axes),
                (dim,),
                self.weight_dtype,
            )
        else:
        ...
        variance = jnp.mean(jnp.square(hidden_states.astype(
            jnp.float32)), axis=-1, keepdims=True)
        hidden_states: jax.Array = hidden_states * \
            jax.lax.rsqrt(variance + self.epsilon)

        if self.elementwise_affine:
            # convert into half-precision if necessary
            hidden_states = (hidden_states.astype(self.dtype)
                             * scale.astype(self.dtype)).astype(input_dtype)
        else:
...
class FeedForward(nn.Module):
    ...
    def __call__(self, hidden_states: jax.Array, scale: float = 1.0, deterministic: bool = False) -> jax.Array:
        ...
        match self.activation_fn:
            ...
            case _:
                raise ValueError(
                    f"activation function {self.activation_fn} not supported")
        ...
        hidden_states = nn.Dropout(self.dropout)(

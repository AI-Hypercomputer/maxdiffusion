...
def get_timestep_embedding_multidim(
    ...
    emb = nn.with_logical_constraint(
        emb, ("activation_batch", "activation_norm_length", "activation_embed"))
    ...
    if flip_sin_to_cos:
        emb = jnp.concatenate(
            [emb[..., half_dim:], emb[..., :half_dim]], axis=-1)
...
class TimestepEmbedding(nn.Module):
    ...
    def __call__(self, sample, condition=None):
        sample = nn.with_logical_constraint(
            sample, ("activation_batch", "activation_norm_length", "activation_embed"))

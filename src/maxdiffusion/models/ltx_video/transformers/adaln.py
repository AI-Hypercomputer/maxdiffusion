...
def get_timestep_embedding_multidim(
    ...
    emb = timesteps[..., None] * emb  # Broadcasting to match shape (*timesteps.shape, half_dim)
    emb = scale * emb
    emb = jnp.concatenate([jnp.sin(emb), jnp.cos(emb)], axis=-1)  # Shape (*timesteps.shape, embedding_dim)
    if flip_sin_to_cos:

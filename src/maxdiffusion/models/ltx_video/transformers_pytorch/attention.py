...
class Attention(nn.Module):
  ...
  def get_processor(self, return_deprecated_lora: bool = False) -> "AttentionProcessor":  # noqa: F821
    ...
    is_lora_activated = {
        name: module.lora_layer is not None for name, module in self.named_modules() if hasattr(module, "lora_layer")
    }
    ...
    if not all(is_lora_activated.values()):
      raise ValueError(f"Make sure that either all layers or no layers have LoRA activated, but have {is_lora_activated}")

...
def generate_cinematic_prompt(
  ...
  if conditioning_items is None:
    ...
    assert len(first_frames) == len(prompts), "Number of conditioning frames must match number of prompts"
...
def _generate_t2v_prompt(
  ...
  model_inputs = prompt_enhancer_tokenizer(texts, return_tensors="pt").to(prompt_enhancer_model.device)
...
def _generate_image_captions(
  ...
  inputs = image_caption_processor(image_caption_prompts, images, return_tensors="pt").to(image_caption_model.device)
...
def _generate_and_decode_prompts(
  ...
  with torch.inference_mode():
    outputs = prompt_enhancer_model.generate(**model_inputs, max_new_tokens=max_new_tokens)
    generated_ids = [output_ids[len(input_ids) :] for input_ids, output_ids in zip(model_inputs.input_ids, outputs)]
    decoded_prompts = prompt_enhancer_tokenizer.batch_decode(generated_ids, skip_special_tokens=True)

...
class ZImageAttention(nnx.Module):
  ...
  def __call__(
    ...
    if self.qk_norm:
      q = self.norm_q(q)
      k = self.norm_k(k)
    q = self._apply_rope(q, freqs_cis, hidden_states.dtype)
    k = self._apply_rope(k, freqs_cis, hidden_states.dtype)
...
class ZImageTransformer2DModel(nnx.Module, ConfigMixin):
  ...
  def __init__(
    ...
    self.final_layer = ZImageFinalLayer(
        rngs, dim, patch_dim, dtype=dtype, weights_dtype=weights_dtype, sharding_specs=self.sharding_specs
    )
  ...
  def __call__(
    ...
    for image, caption in zip(x, cap_feats):
      caption_padded, cap_position, cap_mask = self._pad(caption, (caption.shape[0], 1, 1), (1, 0, 0))
      patch, size, tokens = self._patchify(image)
      patch_padded, image_position, image_mask = self._pad(patch, tokens, (caption.shape[0] + 1, 0, 0))
    ...
    # 2. Find maximum lengths statically (Python values)
    ...
    # 3. Pad to maximum sequence lengths statically
    caption_features_padded, caption_positions_padded, caption_masks_padded = [], [], []
    for cap, pos, mask in zip(caption_features, caption_positions, caption_masks):
      missing = max_caption_len - cap.shape[0]
      if missing == 0:
        caption_features_padded.append(cap)
        caption_positions_padded.append(pos)
        caption_masks_padded.append(mask)
      else:
        caption_features_padded.append(jnp.pad(cap, ((0, missing), (0, 0))))
        caption_positions_padded.append(jnp.pad(pos, ((0, missing), (0, 0))))
        caption_masks_padded.append(jnp.pad(mask, (0, missing), constant_values=True))

    image_features_padded, image_positions_padded, image_masks_padded = [], [], []
    for patch, pos, mask in zip(image_features, image_positions, image_masks):
      missing = max_image_len - patch.shape[0]
      if missing == 0:
        image_features_padded.append(patch)
        image_positions_padded.append(pos)
        image_masks_padded.append(mask)
      else:
        image_features_padded.append(jnp.pad(patch, ((0, missing), (0, 0))))
        image_positions_padded.append(jnp.pad(pos, ((0, missing), (0, 0))))
        image_masks_padded.append(jnp.pad(mask, (0, missing), constant_values=True))

    # Stack into 3D batch arrays (maintains data-parallel sharding natively with zero communication)
    caption_features_stacked = jnp.stack(caption_features_padded, axis=0)
    caption_masks_stacked = jnp.stack(caption_masks_padded, axis=0)
    caption_positions_stacked = jnp.stack(caption_positions_padded, axis=0)

    image_features_stacked = jnp.stack(image_features_padded, axis=0)
    image_masks_stacked = jnp.stack(image_masks_padded, axis=0)
    image_positions_stacked = jnp.stack(image_positions_padded, axis=0)
    ...
    # 5. Rope frequencies
    image_freqs_stacked = self.rope_embedder(image_positions_stacked)
    caption_freqs_stacked = self.rope_embedder(caption_positions_stacked)

    # 6. Refiner blocks
    has_image_padding = any(l < max_image_len for l in image_lengths)
    image_attention_mask = None if not has_image_padding else ~image_masks_stacked
    for block in self.noise_refiner:
      image_embeddings = block(image_embeddings, image_freqs_stacked, image_attention_mask, adaln)

    has_caption_padding = any(l < max_caption_len for l in caption_lengths)
    caption_attention_mask = None if not has_caption_padding else ~caption_masks_stacked
    for block in self.context_refiner:
      captions = block(captions, caption_freqs_stacked, caption_attention_mask)
    ...
    freqs_cis = jnp.concatenate((image_freqs_stacked, caption_freqs_stacked), axis=1)

    # 8. Model layers
    joint_lengths = [img_l + cap_l for img_l, cap_l in zip(image_lengths, caption_lengths)]
    joint_max = max_caption_len + max_image_len
    has_joint_padding = any(l < joint_max for l in joint_lengths)
    joint_attention_mask = (
        None if not has_joint_padding else jnp.concatenate((~image_masks_stacked, ~caption_masks_stacked), axis=1)
    )

    for block in self.layers:
      joint = block(joint, freqs_cis, joint_attention_mask, adaln)

...
class FlaxLTX2AudioAttnBlock(nnx.Module):
  ...
  def __init__(
    ...
    self.proj_out = nnx.Conv(
        in_channels, in_channels, kernel_size=(1, 1), strides=(1, 1), padding="VALID", dtype=dtype, rngs=rngs
    )
...
class FlaxLTX2AudioAudioPatchifier:
  ...
  def patchify(self, audio_latents: jnp.ndarray) -> jnp.ndarray:
    """
    Converts a 4D audio latent tensor into a 3D patched sequence.
    Args:
        audio_latents (`jnp.ndarray`): Input tensor of shape `(batch, time, freq, channels)`.
    Returns:
        `jnp.ndarray`: Patched tensor of shape `(batch, time, channels * freq)`.
    """
  ...
  def unpatchify(self, patched_latents: jnp.ndarray, channels: int, freq: int) -> jnp.ndarray:
    """
    Converts a 3D patched sequence back into a 4D audio latent tensor.
    Args:
        patched_latents (`jnp.ndarray`): Input patched tensor of shape `(batch, time, channels * freq)`.
        channels (`int`): The number of channels to restore.
        freq (`int`): The frequency dimension size to restore.
    Returns:
        `jnp.ndarray`: Restored tensor of shape `(batch, time, freq, channels)`.
    """
...
class FlaxLTX2AudioEncoder(nnx.Module):
  ...
  def __init__(
    ...
    if self.causality_axis is not None and self.causality_axis != "none":
      self.conv_in = FlaxLTX2AudioCausalConv(
          in_channels, base_block_channels, kernel_size=3, causality_axis=self.causality_axis, dtype=dtype, rngs=rngs
      )
    else:
    ...
    for level in range(len(self.ch_mult)):
      ...
      if level != len(self.ch_mult) - 1:
        downsample = FlaxLTX2AudioDownsample(
            block_in, with_conv=True, causality_axis=self.causality_axis, dtype=dtype, rngs=rngs
        )
        curr_res = curr_res // 2
    ...
    self.mid_attn = (
        FlaxLTX2AudioAttnBlock(block_in, norm_type=norm_type, dtype=dtype, rngs=rngs) if mid_block_add_attention else None
    )
    self.mid_block2 = FlaxLTX2AudioResnetBlock(
        block_in,
        block_in,
        temb_channels=0,
        dropout=dropout,
    ...
    if self.causality_axis is not None and self.causality_axis != "none":
      self.conv_out = FlaxLTX2AudioCausalConv(
          block_in, z_channels, kernel_size=3, causality_axis=self.causality_axis, dtype=dtype, rngs=rngs
      )
    else:
...
class FlaxLTX2AudioDecoder(nnx.Module):
  ...
  def __init__(
    ...
    if self.causality_axis is not None and self.causality_axis != "none":
      self.conv_in = FlaxLTX2AudioCausalConv(
          latent_channels, base_block_channels, kernel_size=3, causality_axis=self.causality_axis, dtype=dtype, rngs=rngs
      )
    else:
      self.conv_in = nnx.Conv(
          latent_channels, base_block_channels, kernel_size=(3, 3), padding="SAME", dtype=dtype, rngs=rngs
      )
    ...
    self.mid_attn = (
        FlaxLTX2AudioAttnBlock(base_block_channels, norm_type=self.norm_type, dtype=dtype, rngs=rngs)
    ...
    for level in reversed(range(len(self.ch_mult))):
      ...
      upsample = None
      if level != 0:
        upsample = FlaxLTX2AudioUpsample(
            block_in, with_conv=True, causality_axis=self.causality_axis, dtype=dtype, rngs=rngs
        )
        curr_res *= 2
    ...
    if self.causality_axis is not None and self.causality_axis != "none":
      self.conv_out = FlaxLTX2AudioCausalConv(
          block_in, self.output_channels, kernel_size=3, causality_axis=self.causality_axis, dtype=dtype, rngs=rngs
      )
    else:
...
class FlaxAutoencoderKLLTX2Audio(nnx.Module, FlaxModelMixin, ConfigMixin):
  ...
  def __init__(
    ...
    # Compute downsample factor dynamically based on ch_mult
    self.latent_downsample_factor = 2 ** (len(self.ch_mult) - 1)
    ...
    self.patchifier = FlaxLTX2AudioAudioPatchifier(
        ...
        audio_latent_downsample_factor=self.latent_downsample_factor,
        is_causal=is_causal,
    )
  ...
  def decode(self, z: jnp.ndarray, return_dict: bool = True, train: bool = False):
    ...
    if self.causality_axis is not None and self.causality_axis != "none":
      target_frames = max(target_frames - (self.latent_downsample_factor - 1), 1)

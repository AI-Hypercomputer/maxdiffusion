...
class UpsampleType(str, Enum):
  SPATIOTEMPORAL = "spatiotemporal"
  TEMPORAL = "temporal"
  SPATIAL = "spatial"


class DownsampleType(str, Enum):
  CONV = "conv"
  SPATIAL = "spatial"
  TEMPORAL = "temporal"
  SPATIOTEMPORAL = "spatiotemporal"
...
class PerChannelRMSNorm(nnx.Module):
  ...
  def __init__(
      self, channel_dim: int = -1, eps: float = 1e-8, rngs: Optional[nnx.Rngs] = None, dtype: jnp.dtype = jnp.float32
  ):
    self.eps = eps
...
class LTX2VideoCausalConv3d(nnx.Module):
  ...
  def __call__(self, hidden_states: jax.Array, causal: bool = True) -> jax.Array:
    ...
    if pad_h > 0 or pad_w > 0:
      padding_config = ((0, 0), (0, 0), (pad_h, pad_h), (pad_w, pad_w), (0, 0))  # B  # T  # H  # W  # C
  ...
  def __init__(
    ...
    if in_channels != out_channels:
      self.norm3 = nnx.LayerNorm(
          in_channels, epsilon=eps, use_scale=True, use_bias=True, rngs=rngs, dtype=dtype, param_dtype=weights_dtype
      )
...
class LTX2VideoDownBlock3D(nnx.Module):
  ...
  def __init__(
      ...
      downsample_type: DownsampleType = DownsampleType.CONV,
      spatial_padding_mode: str = "constant",
      rngs: Optional[nnx.Rngs] = None,
      mesh: Optional[jax.sharding.Mesh] = None,
      dtype: jnp.dtype = jnp.float32,
  ...
  def __init__(
    ...
    if timestep_conditioning:
      self.time_embedder = nnx.data(
          NNXPixArtAlphaCombinedTimestepSizeEmbeddings(
              rngs=rngs,
...
class LTX2VideoUpBlock3d(nnx.Module):
  ...
  def __init__(
      ...
      upscale_factor: int = 1,
      upsample_type: UpsampleType = UpsampleType.SPATIOTEMPORAL,
      spatial_padding_mode: str = "constant",
      rngs: Optional[nnx.Rngs] = None,
      mesh: Optional[jax.sharding.Mesh] = None,
      dtype: jnp.dtype = jnp.float32,
...
class LTX2VideoEncoder3d(nnx.Module):
  ...
  def __init__(
      ...
      downsample_type: Tuple[DownsampleType, ...] = (
          DownsampleType.SPATIAL,
          DownsampleType.TEMPORAL,
          DownsampleType.SPATIOTEMPORAL,
          DownsampleType.SPATIOTEMPORAL,
      ),
      patch_size: int = 4,
      patch_size_t: int = 1,
...
class LTX2VideoDecoder3d(nnx.Module):
  ...
  def __init__(
      ...
      upsample_type: Tuple[UpsampleType, ...] = (
          UpsampleType.SPATIOTEMPORAL,
          UpsampleType.SPATIOTEMPORAL,
          UpsampleType.SPATIOTEMPORAL,
      ),
...
class LTX2VideoAutoencoderKL(nnx.Module, FlaxModelMixin, ConfigMixin):
  ...
  def __init__(
    ...
    self.spatial_compression_ratio = (
        patch_size * 2 ** sum(spatio_temporal_scaling) if spatial_compression_ratio is None else spatial_compression_ratio
    )
  ...
  def _temporal_tiled_encode(
      self, x: jax.Array, key: Optional[jax.Array] = None, causal: Optional[bool] = None
  ) -> jax.Array:
    B, T, H, W, C = x.shape

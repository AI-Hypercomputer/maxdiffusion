...
class LTX2VideoTransformer3DModel(nnx.Module, ConfigMixin):
  ...
  def __call__(
    ...
    with jax.named_scope("Transformer Blocks"):
      ...
      def apply_block(block, context: LTX2BlockContext, mask, rngs_carry):
        orig_perturbation_mask = context.perturbation_mask
        context = dataclasses.replace(context, perturbation_mask=mask)
        ...
        return context, rngs_carry

      if perturbation_mask is None:

        def scan_fn_ltx2(carry, block):
          context, rngs_carry = carry
          context, rngs_carry = apply_block(block, context, None, rngs_carry)
        ...
        if self.scan_layers:
          ...
          for block in self.transformer_blocks:
            current_context, _ = apply_block(block, current_context, None, None)
        ...
        def scan_fn_ltx23(carry, block_and_mask):
          ...
          context, rngs_carry = carry
          context, rngs_carry = apply_block(block, context, mask, rngs_carry)
          return (context, rngs_carry), None
        ...
        if self.scan_layers:
          ...
          for i, block in enumerate(self.transformer_blocks):
            ...
            current_context, _ = apply_block(block, current_context, mask, None)
          hidden_states = current_context.hidden_states
          audio_hidden_states = current_context.audio_hidden_states

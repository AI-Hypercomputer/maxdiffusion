...
class FlaxQwen3Attention(nn.Module):
    ...
    def __call__(
        ...
        scores = jnp.matmul(q, jnp.transpose(k, (0, 1, 3, 2))) / math.sqrt(self.config.head_dim)

        # 7. Apply causal attention mask
        causal_mask = jnp.tril(jnp.ones((seq_len, seq_len), dtype=self.config.dtype))
        mask_value = jnp.where(causal_mask, 0.0, -1e10)
        ...
        if attention_mask is not None:
            ...
            p_mask = attention_mask[:, jnp.newaxis, jnp.newaxis, :]
            p_mask_value = jnp.where(p_mask, 0.0, -1e10)
            scores = scores + p_mask_value

        # 9. Softmax & Weighted Sum
        probs = jax.nn.softmax(scores, axis=-1)
        out = jnp.matmul(probs, v)  # (batch, num_heads, seq_len, head_dim)

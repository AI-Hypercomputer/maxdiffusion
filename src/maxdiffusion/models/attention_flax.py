  ...
  def wrap_ulysses_attention(query, key, value, attention_mask):
    ...
    if use_custom_kernel:
      ...
      if use_fixed_m:
        ...
        attention_output = jax.lax.cond(all_fixed, _run_uniform, _run_hybrid, query, key, value, mk_arr, k_mean)
      else:
...
def _ulysses_ring_attention(
  ...
  _warn_if_ring_is_degenerate(num_ring_shards, num_ulysses_shards, num_context_shards, heads=heads, kv_heads=kv_heads)
...
def _ulysses_ring_custom_attention(
  ...
  # Ring fixed-m is enabled here for all R. The previous change restricted it to
  # R=1 because the caller could only bound the raw, uncentered K while the ring
  # kernel exponentiates centered logits q . (k_j - k_mean). This change plumbs
  # the same global k_mean through to the caller and derives the Cauchy-Schwarz
  # bound from the centered keys (see kf_centered below), so the bound once
  # again caps the quantity it is checked against and the restriction is lifted.
  _warn_if_ring_is_degenerate(num_ring_shards, num_ulysses_shards, num_context_shards, heads=heads, kv_heads=kv_heads)
  query, orig_q_seq_len = _reshape_data_for_flash(query, heads, num_context_shards)
  ...
  def wrap_ulysses_ring_attention(query, key, value):
    ...
    k_mean = None
    if use_fixed_m:
      # Global Virtual K-Centering:
      # Compute local key mean after Ulysses A2A on real unpadded tokens, then all-reduce across ring axis if R > 1
      k_mean_local = jnp.mean(real_key.astype(jnp.float32), axis=2)
      if num_ring_shards > 1:
        k_mean = jax.lax.pmean(k_mean_local, axis_name=ring_axis)
      else:
        k_mean = k_mean_local
      if k_mean.shape[-1] < 128:
        k_mean = jnp.pad(k_mean, ((0, 0), (0, 0), (0, 128 - k_mean.shape[-1])))
    ...
    query, kv_size, query_seq_len = _pad_data_for_flash(raw_query, heads, bq)
    # When num_ring_shards == 1, K/V are passed with NO sequence padding. The
    # fixed-m kernel slices the ragged KV tail using slice lengths derived from
    # the unpadded actual_kv_seq_len, avoiding 2 x 185MB of HBM traffic per layer.
    kv_pad_size = 1 if num_ring_shards == 1 else bkv
    ...
    mk_arr, all_fixed, qn_dev, mk_dev = None, None, None, None
    if use_fixed_m:
      if num_ring_shards == 1:
        recenter, safe_bound = custom_splash.get_fixed_m_constants(actual_kv_seq_len, is_ring=False)
        mk_arr, all_fixed = _compute_fixed_m_metadata(
        ...
        mk_dev = jnp.sum(kf_centered**2, axis=-1).max(axis=-1)
        if num_q_heads != num_kv_heads:
          if num_q_heads % num_kv_heads != 0:
            raise ValueError(
          ...
          q_heads_per_kv_head = num_q_heads // num_kv_heads
          mk_dev = jnp.repeat(mk_dev, q_heads_per_kv_head, axis=1)
          k_mean = jnp.repeat(k_mean, q_heads_per_kv_head, axis=1)
    ...
    # (2b) Ring: Cross-chip ppermute schedule with custom ring kernel
      ...
      if use_fixed_m:
        ...
        v_ok = jax.lax.pmin(v_ok_local, ring_axis)
...
def _apply_attention_dot(
  ...
  if split_head_dim:
    ...
    if heads != effective_kv_heads:
      ...
      value_states = jnp.repeat(value_states.reshape(b, effective_kv_heads, s_k, -1), num_repeats, axis=1).reshape(
          b * heads, s_k, -1
      )

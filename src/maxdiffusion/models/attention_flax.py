...
def _warn_if_ring_is_degenerate(
  ...
  _warn_once(
      ...
      f"This ring variant is degenerate: no KV is rotated, the cross-ring mean for virtual K-centering "
      f"is skipped, and the result is mathematically identical to the corresponding non-ring "
      f"'ulysses_custom*' kernel. Do NOT report this as a ring-attention result. " + advice,
  )
...
def _ulysses_attention(
  ...
  def wrap_ulysses_attention(query, key, value, attention_mask):
    ...
    if use_custom_kernel:
      ...
      if use_fixed_m:
        ...
        # Computed strictly on real (unpadded) tokens.
        k_mean = jnp.mean(real_key.astype(jnp.float32), axis=2)
        pad_h = (custom_splash.NUM_SUBLANES - (k_mean.shape[1] % custom_splash.NUM_SUBLANES)) % custom_splash.NUM_SUBLANES
        pad_d = max(0, query.shape[-1] - k_mean.shape[-1])
        if pad_h > 0 or pad_d > 0:
          k_mean = jnp.pad(k_mean, ((0, 0), (0, pad_h), (0, pad_d)))
      # K/V are passed with NO sequence padding. The fixed-m kernel slices the
      # ragged KV tail (`last_compute_body_fixed` in custom_splash_attention.py)
      # using slice lengths derived from the unpadded `orig_kv_seq_len`, so it
      # never reads a padded K/V row; materialising the pad cost 2 x 185MB of HBM
      # traffic per layer for nothing. Passing flash_block_size=1 makes only the
      # sequence pad a no-op -- the head_dim->128 pad, the reshape and the
      # returned (tensor, kv_size, seq_len) contract are all preserved, so
      # configs with head_dim < 128 still behave exactly as before.
      key, _, key_seq_len = _pad_data_for_flash(raw_key, heads, 1)
      value, _, _ = _pad_data_for_flash(raw_value, heads, 1)
  ...
  out_q_axis_names = (
      jax.sharding.PartitionSpec(q_axis_names[0], q_axis_names[1], q_axis_names[3], q_axis_names[2])
      if use_custom_kernel
      else q_axis_names
  )
...
def _max_row_norm_per_head(x: jax.Array) -> jax.Array:
  """Largest row L2 norm estimate per head of a `[..., H, S, D]` activation in FP32 with conservative exponent margin, preserving batch dims."""
  row_sq = (x.astype(jnp.float32) ** 2).sum(axis=-1)
  return jnp.sqrt(row_sq.max(axis=-1))
...
def _ring_fixed_m_norms_pre_a2a(
  ...
  `all_fixed_global`) is materially cheaper:
    ...
    * All reductions and cross-chip `pmin`/`pmax` collectives become
      independent of `all_to_all(query, key, value)`, allowing XLA's
      latency-hiding scheduler to overlap them with the all-to-all instead of
      serialising `a2a -> post-a2a V reduce -> ring pmin -> ring pmin -> lax.cond`.
  ...
  #
  # This used to be two collectives -- a pmax of (Q, V) over (ulysses, ring),
  # then `all_gather(pmax(kn_local, ulysses), ring)` to give every hop its own
  # K shard's norm. The gather is what made this expensive, and not for the
  # reason one would guess: its payload is only `heads` floats. Profiling shows
  # the collective time is unchanged by removing it; what changes is *compute*.
  # On TPU the ring-axis gather forces a relayout of the small norm arrays that
  # sits in the middle of the QKV projection's fusion region, and XLA then fails
  # to fuse across it. Removing it recovered ~2.5 s per 40-step denoise
  # (convolution fusion -1.64 s, loop fusion -0.44 s, data formatting -0.36 s).
  qn_head_global, vn_global, kn_global = jax.lax.pmax((qn_head_local, vn_local, kn_local), axis_name=reduce_axes)

  if not per_q_block:
    qn_all = qn_head_global
  else:
    ...
    scattered = jnp.zeros((batch, num_q_heads, padded_seq), q_norm_sq.dtype)
    seq_offset = jax.lax.axis_index(ulysses_axis) * local_seq
    scattered = jax.lax.dynamic_update_slice_in_dim(scattered, q_norm_sq, seq_offset, axis=2)
    qn_local = scattered.reshape(batch, num_q_heads, num_q_blocks, block_q).max(axis=-1)
    qn_all = jax.lax.pmax(qn_local, ulysses_axis)

  # `mk_all_sq` keeps its (batch, ring, heads) shape so the kernel's per-hop
  # interface is unchanged, but every hop now carries the ring-wide max rather
  # than that hop's own shard norm. Using the max over hops can only enlarge the
  # Cauchy-Schwarz bound, so fixed-m eligibility becomes *more* conservative,
  # never less -- and it is what `mk_global_sq` below already collapsed it to,
  # so `all_fixed_global` is bit-identical to the per-hop version.
  ...
  # Because qn_head_global_dev, vn_dev, and mk_global_sq are already reduced over
  # ring_axis, both v_ok and all_fixed_global are bit-identical across all ring
  # ranks with zero post-all_gather collectives.
  effective_kv_seq_len = key.shape[2] * num_ulysses_shards * num_ring_shards
  global_recenter, global_bound = custom_splash.get_fixed_m_constants(effective_kv_seq_len, is_ring=True)
...
def _compute_fixed_m_metadata(
  ...
  Args:
    query: Padded query activation, shape `(batch, local_heads, padded_q_len, head_dim)`.
    key: Key activation (raw unpadded or padded), shape `(batch, local_heads, kv_len, head_dim)`.
...
def _ulysses_ring_custom_attention(
  ...
  # Virtual K-centering is switched ON for the ring path here. The kernel only
  # centers when it is handed a `k_mean` (it stays uncentered otherwise, which is
  # what the preceding change ships), so enabling it means supplying both halves
  # of the matched pair: the global mean itself, and a Cauchy-Schwarz bound
  # derived from the centered keys rather than the raw ones (see kf_centered
  # below). Centering shrinks the bound, which is what lets more heads stay on
  # the fixed-m path at R > 1.
  _warn_if_ring_is_degenerate(
      num_ring_shards,
      num_ulysses_shards,
      num_context_shards,
  ...
  def wrap_ulysses_ring_attention(query, key, value):
    ...
    query, kv_size, query_seq_len = _pad_data_for_flash(raw_query, heads, bq)
    # When num_ring_shards == 1 or actual_kv_seq_len is aligned to 8 sublanes, K/V
    # are passed with NO sequence padding. The kernel slices the ragged KV tail
    # using slice lengths derived from actual_kv_seq_len, avoiding sequence pad
    # HBM copies and redundant ppermute/MXU compute on padded keys.
    kv_pad_size = 1 if (num_ring_shards == 1 or actual_kv_seq_len % 8 == 0) else bkv
...
def ulysses_ring_custom_fixed_m_kernel(q, k, v, context):
  ...
  return _ulysses_ring_custom_attention(
      ...
      kv_heads=context.get("kv_heads", None),
      use_k_centering=context.get("use_k_centering", False),
  )
...
class NNXAttentionOp(nnx.Module):
  ...
  def apply_attention(
    ...
    return _apply_attention(
        ...
        use_k_centering=self.use_k_centering if hasattr(self, "use_k_centering") else False,
    )
...
class FlaxWanAttention(nnx.Module):
  ...
  def _apply_rope(self, xq: jax.Array, xk: jax.Array, freqs_cis: jax.Array) -> Tuple[jax.Array, jax.Array]:
    ...
    # 5. Concatenate along last axis instead of stack + reshape to prevent layout fragmentation

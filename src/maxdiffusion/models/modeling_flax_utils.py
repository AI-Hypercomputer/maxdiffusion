...
class FlaxModelMixin(PushToHubMixin):
  ...
  def init_weights(self, rng: jax.Array, eval_only: bool = False) -> Dict:
  ...
  def from_pretrained(
    ...
    pretrained_path_with_subfolder = (
        pretrained_model_name_or_path if subfolder is None else os.path.join(pretrained_model_name_or_path, subfolder)
    )
    if os.path.isdir(pretrained_path_with_subfolder):
      ...
      try:
        ...
        raise EnvironmentError(f"{pretrained_model_name_or_path} does not appear to have a file named {FLAX_WEIGHTS_NAME}.")

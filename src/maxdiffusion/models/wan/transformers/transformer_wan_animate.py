...
class MotionConv2d(nnx.Module):
    ...
    def __init__(
        ...
        if blur_kernel is not None:
            ...
            self.blur_kernel = nnx.static(
                tuple(tuple(float(v) for v in row) for row in kernel)
            )
            self.blur = True
        else:
...
class MotionLinear(nnx.Module):
    ...
    def __init__(
        ...
        key = rngs.params()
        self.weight = nnx.Param(
            jax.random.normal(key, (out_dim, in_dim), dtype=weights_dtype)
        )
...
class WanAnimateMotionEncoder(nnx.Module):
    ...
    def __init__(
        ...
        for _ in range(motion_blocks - 1):
            linears.append(
                MotionLinear(
                    rngs, style_dim, style_dim, dtype=dtype, weights_dtype=weights_dtype
                )
            )

        linears.append(
            MotionLinear(
                rngs, style_dim, motion_dim, dtype=dtype, weights_dtype=weights_dtype
            )
        )
        self.motion_network = nnx.List(linears)

        key = rngs.params()
        self.motion_synthesis_weight = nnx.Param(
            jax.random.normal(key, (out_dim, motion_dim), dtype=weights_dtype)
        )
...
class WanAnimateFaceEncoder(nnx.Module):
    def __init__(
        ...
        self.out_proj = nnx.Linear(
            ...
            kernel_init=nnx.with_partitioning(
                nnx.initializers.lecun_normal(), ("mlp", "embed")
            ),
            bias_init=nnx.with_partitioning(nnx.initializers.zeros, ("embed",)),
        )

        self.padding_tokens = nnx.Param(
            jnp.zeros((1, 1, 1, out_dim), dtype=weights_dtype)
        )
    ...
    def __call__(self, x: jax.Array) -> jax.Array:
        ...
        padding = jnp.broadcast_to(
            self.padding_tokens[...], (batch_size, x.shape[1], 1, self.out_dim)
        )
...
class WanAnimateFaceBlockCrossAttention(nnx.Module):
    def __init__(
        ...
        self.pre_norm_q = nnx.LayerNorm(
            dim, epsilon=eps, use_bias=False, use_scale=False, rngs=rngs, dtype=dtype
        )
        self.pre_norm_kv = nnx.LayerNorm(
            dim, epsilon=eps, use_bias=False, use_scale=False, rngs=rngs, dtype=dtype
        )
        ...
        self.to_q = nnx.Linear(
            ...
            param_dtype=weights_dtype,
            kernel_init=nnx.with_partitioning(
                nnx.initializers.lecun_normal(), ("embed", "heads")
            ),
        ...
        self.to_out = nnx.Linear(
            ...
            param_dtype=weights_dtype,
            kernel_init=nnx.with_partitioning(
                nnx.initializers.lecun_normal(), ("heads", "embed")
            ),
            bias_init=nnx.with_partitioning(nnx.initializers.zeros, ("embed",)),
        )
...
class WanAnimateTransformer3DModel(nnx.Module, FlaxModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        self.scale_shift_table = nnx.Param(
            jax.random.normal(key, (1, 2, inner_dim), dtype=weights_dtype)
            / inner_dim**0.5,
            kernel_init=nnx.with_partitioning(
    ...
    def conditional_named_scope(self, name: str):
        return (
            ...
            if self.enable_jax_named_scopes
            else contextlib.nullcontext()
        )

    def _apply_face_adapter(
        self, hidden_states: jax.Array, motion_vec: Optional[jax.Array], block_idx
    ) -> jax.Array:
        ...
        adapter_branches = tuple(
            (
                lambda current_hidden_states, adapter=adapter: current_hidden_states
                + adapter(current_hidden_states, motion_vec)
            )
    ...
    def __call__(
        ...
        hidden_states = jnp.reshape(
            hidden_states, (batch_size, -1, hidden_states.shape[-1])
        )
        ...
        face_pixel_values = jnp.reshape(
            face_pixel_values, (-1, face_channels, face_height, face_width)
        )
        ...
        pad_len = (
            motion_encode_batch_size - (total_face_frames % motion_encode_batch_size)
        ) % motion_encode_batch_size
        if pad_len > 0:
        ...
        if self.scan_layers:
            ...
            def scan_fn(carry, block_idx, block):
                ...
                hidden_states = jax.lax.cond(
                    ...
                    lambda current_hidden_states: self._apply_face_adapter(
                        current_hidden_states, motion_vec, block_idx
                    ),
            ...
            for block_idx, block in enumerate(self.blocks):
                ...
                def layer_forward(hidden_states):
                    ...
                    if (
                        ...
                        hidden_states = self._apply_face_adapter(
                            hidden_states, motion_vec, block_idx
                        )
                    return hidden_states
        ...
        hidden_states = jnp.reshape(
            hidden_states, (batch_size, -1, num_frames, height, width)
        )

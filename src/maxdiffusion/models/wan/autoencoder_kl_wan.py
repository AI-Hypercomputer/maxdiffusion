...
class WanCausalConv3d(nnx.Module):
    ...
    def __init__(
        ...
        padding_tuple = _canonicalize_tuple(
            padding, 3, "padding"
        ...
        self._causal_padding = (
            ...
            (
                2 * padding_tuple[0],
                0,
            ),  # Depth dimension - causal padding (pad only before)
            (
                padding_tuple[1],
                padding_tuple[1],
            ),  # Height dimension - symmetric padding
    ...
    def __call__(
        ...
        if any(p > 0 for dim_pads in padding_to_apply for p in dim_pads):
            x_padded = jnp.pad(
                x_input, padding_to_apply, mode="constant", constant_values=jnp.array(0.0, dtype=x_input.dtype)
            )
        else:
            x_padded = x_input

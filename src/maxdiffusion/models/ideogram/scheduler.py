...
class LogitNormalSchedule:
  ...
  def __call__(self, t: jax.Array) -> jax.Array:
    ...
    z = jax.scipy.special.ndtri(t)
    y = self.mean + self.std * z
    t_ = jax.scipy.special.expit(y)
    t_ = 1 - t_
    t_min = 1.0 / (1 + math.exp(0.5 * self.logsnr_max))
    t_max = 1.0 / (1 + math.exp(0.5 * self.logsnr_min))
    return jnp.clip(t_, t_min, t_max).astype(jnp.float32)
...
def make_step_intervals(num_steps: int) -> jax.Array:
  """Default linear step schedule used by the v4 eval config."""
  return jnp.linspace(0.0, 1.0, num_steps + 1, dtype=jnp.float32)

...
def swap_linears_to_bnb4bit(
  ...
  if bnb is None:
    raise ImportError("bitsandbytes is not installed. 4-bit quantization is not available on this device.")
...
class Fp8Linear(nn.Module):
  ...
  def dequantize(self, dtype: torch.dtype | None = None) -> torch.Tensor:
    ...
    if (
        ...
        and self._cached_weight.dtype == target_dtype
        and self._cached_weight.device == self.weight.device
    ):
      return self._cached_weight
    self._cached_weight = self.weight.to(target_dtype) * self.weight_scale.to(target_dtype).unsqueeze(1)
    return self._cached_weight

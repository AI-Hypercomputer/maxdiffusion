...
class AttnBlock(nnx.Module):
  ...
  def __init__(self, rngs: nnx.Rngs, in_channels: int):
    ...
    self.norm = nnx.GroupNorm(in_channels, num_groups=32, epsilon=1e-6, use_bias=True, use_scale=True, rngs=rngs)
...
class ResnetBlock(nnx.Module):
  ...
  def __init__(self, rngs: nnx.Rngs, in_channels: int, out_channels: int):
    ...
    self.norm1 = nnx.GroupNorm(in_channels, num_groups=32, epsilon=1e-6, rngs=rngs)
    self.conv1 = nnx.Conv(in_channels, out_channels, kernel_size=(3, 3), strides=(1, 1), padding=((1, 1), (1, 1)), rngs=rngs)

    self.norm2 = nnx.GroupNorm(out_channels, num_groups=32, epsilon=1e-6, rngs=rngs)
    self.conv2 = nnx.Conv(
        out_channels, out_channels, kernel_size=(3, 3), strides=(1, 1), padding=((1, 1), (1, 1)), rngs=rngs
    )
...
class Encoder(nnx.Module):
  ...
  def __init__(
    ...
    for i_level in range(self.num_resolutions):
      ...
      down_blocks.append(nnx.List(block))
      down_attns.append(nnx.List(attn))
      downsamples.append(downsample)

    self.down_blocks = nnx.List(down_blocks)
    self.down_attns = nnx.List(down_attns)
    ...
    self.norm_out = nnx.GroupNorm(block_in, num_groups=32, epsilon=1e-6, rngs=rngs)
    self.conv_out = nnx.Conv(
        block_in, 2 * z_channels, kernel_size=(3, 3), strides=(1, 1), padding=((1, 1), (1, 1)), rngs=rngs
    )

  def __call__(self, x: jax.Array) -> jax.Array:
    hs = [self.conv_in(x)]
    for i_level in range(self.num_resolutions):
      block = self.down_blocks[i_level]
      attn = self.down_attns[i_level]
      downsample = self.downsamples[i_level]
...
class Decoder(nnx.Module):
  ...
  def __init__(
    ...
    for i_level in reversed(range(self.num_resolutions)):
      ...
      up_blocks.insert(0, nnx.List(block))
      up_attns.insert(0, nnx.List(attn))
      upsamples.insert(0, upsample)

    self.up_blocks = nnx.List(up_blocks)
    self.up_attns = nnx.List(up_attns)
  ...
  def __call__(self, z: jax.Array) -> jax.Array:
    ...
    for i_level in reversed(range(self.num_resolutions)):
      block = self.up_blocks[i_level]
      attn = self.up_attns[i_level]
      upsample = self.upsamples[i_level]

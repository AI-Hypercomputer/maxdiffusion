...
def parse_lora_dict(state_dict):
    ...
    for k, v in state_dict.items():
        ...
        if k.endswith(".diff_b"):
            ...
            if key_base not in lora_params: lora_params[key_base] = {}
            lora_params[key_base]["diff_b"] = _to_jax_array(v)
            continue
        ...
        if k.endswith(".diff"):
            key_base = k[:-len(".diff")]
            if key_base not in lora_params: lora_params[key_base] = {}
            lora_params[key_base]["diff"] = _to_jax_array(v)
            continue

        # Standard LoRA
        m = re.match(r"^(.*?)_lora\.(down|up)\.weight$", k)
        if not m: m = re.match(r"^(.*?)\.lora\.(down|up)\.weight$", k)
        if not m: m = re.match(r"^(.*?)\.(lora_down|lora_up)\.weight$", k)
...
def merge_lora(model: nnx.Module, state_dict: dict, scale: float, translate_fn=None):
    ...
    """
    lora_params = parse_lora_dict(state_dict)
        ...
        if lora_key and lora_key in lora_params:
            ...
            if down_w is not None or w_diff is not None or b_diff is not None:
                ...
                assigned_count +=1
                 ...
                 max_logging.log(f"Matched key {lora_key} but found no actionable weights.")

    max_logging.log(f"Merged weights into {assigned_count} layers.")
    unmatched_keys = set(lora_params.keys()) - matched_keys
    if unmatched_keys:
        max_logging.log(f"{len(unmatched_keys)} key(s) in LoRA dictionary were not applied to any layer in the model: {unmatched_keys}")
    ...
    for path, module in nnx.iter_graph(model):
        ...
        if not is_scanned:
            ...
            if lora_key in lora_params:
                matched_keys.add(lora_key)
                weights = lora_params[lora_key]
                is_conv_kxk_locon = isinstance(module, nnx.Conv) and module.kernel_size != (1,1) and "down" in weights and "up" in weights
                ...
                if isinstance(module, nnx.Embed):
                    ...
                    down_w, up_w, current_scale_ = None, None, None
                    if "down" in weights and "up" in weights and not is_conv_kxk_locon:
                        down_w, up_w = np.array(weights["down"]), np.array(weights["up"])
                        if isinstance(module, nnx.Conv): down_w, up_w = np.squeeze(down_w), np.squeeze(up_w)
                        rank, alpha = down_w.shape[0], float(weights.get("alpha", down_w.shape[0]))
                        current_scale_ = scale * alpha / rank
                    
                    w_diff, b_diff = weights.get("diff", None), weights.get("diff_b", None)
                    if w_diff is not None:
                        w_diff = np.array(w_diff)
                        if isinstance(module, nnx.Conv):
                            if w_diff.ndim == 5: w_diff = w_diff.transpose((2,3,4,1,0))
                            elif w_diff.ndim == 4: w_diff = w_diff.transpose((2,3,1,0))
                        elif isinstance(module, nnx.Linear) and w_diff.ndim == 2:
                            w_diff = w_diff.transpose((1,0))
                    if b_diff is not None: b_diff = np.array(b_diff)
                    if is_conv_kxk_locon:
                        ...
                        lora_delta = delta_fx * (scale * alpha / rank)
                        if w_diff is None: w_diff = lora_delta.astype(np.float32)
                        else: w_diff += lora_delta.astype(w_diff.dtype)
                    
                    bias_val = module.bias.value if module.bias is not None else None
                    if down_w is not None or w_diff is not None or b_diff is not None:
                        k, b = _compute_and_add_single_jit(module.kernel.value, bias_val, down_w, up_w, current_scale_, w_diff, b_diff)
                        module.kernel.value = k
                        if b is not None: module.bias.value = b
                        assigned_count +=1
            continue

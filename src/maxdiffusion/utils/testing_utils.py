...
def require_torch_2(test_case):
  ...
  """
  return unittest.skipUnless(is_torch_available() and is_torch_version(">=", "2.0.0"), "test requires PyTorch 2")(test_case)
...
def load_hf_numpy(path) -> np.ndarray:
  if not path.startswith("http://") or path.startswith("https://"):
    path = os.path.join("https://huggingface.co/datasets/fusing/diffusers-testing/resolve/main", urllib.parse.quote(path))
...
class CaptureLogger:
  ...
  Example:
  ```python
  >>> from maxdiffusion import logging
  >>> from maxdiffusion.testing_utils import CaptureLogger

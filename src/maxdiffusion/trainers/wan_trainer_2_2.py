...
def compute_unshifted_boundary(boundary_ratio: float, shift: float) -> float:
  """Computes unshifted continuous time boundary u in [0, 1] corresponding to a shifted boundary_ratio.

  In Flow Matching, time shift S(u) = (shift * u) / (1 + (shift - 1) * u) maps uniform latent time u in [0, 1]
  to shifted continuous time t in [0, 1]. During inference, expert routing checks shifted timestep
  t >= boundary_ratio * num_train_timesteps.
  To align training and inference so that both experts train on their exact inference timestep domains,
  the boundary in the uniform unshifted space is the inverse shift S^{-1}(boundary_ratio):
    u_boundary = boundary_ratio / (shift - (shift - 1) * boundary_ratio).
  """
  if shift != 1.0:
    return float(boundary_ratio / (shift - (shift - 1.0) * boundary_ratio))
...
class WanTrainer2_2(BaseWanTrainer):
  ...
  def create_scheduler(self):
    """Creates and initializes the Flow Match scheduler with configured flow_shift."""
  ...
  def get_data_shardings(self, mesh):
    sharding = jax.sharding.NamedSharding(mesh, P(*self.config.data_sharding))
    return {"latents": sharding, "encoder_hidden_states": sharding}
  ...
  def get_train_step(self, pipeline, mesh, state_shardings, data_shardings):
    return jax.jit(
        ...
        in_shardings=(state_shardings["low_noise"], state_shardings["high_noise"], data_shardings, None, None, None),
  ...
  def start_training(self):
    ...
    optimizer_low, learning_rate_scheduler_low = self.checkpointer._create_optimizer(
        pipeline.low_noise_transformer, self.config, self.config.learning_rate, scale_factor=u_boundary
    )
    optimizer_high, learning_rate_scheduler_high = self.checkpointer._create_optimizer(
        pipeline.high_noise_transformer,
        self.config,
        self.config.learning_rate,
        scale_factor=(1.0 - u_boundary),
    )
  ...
  def eval_2_2(self, mesh, eval_rng_key, step, p_eval_step, state_low, state_high, scheduler_state, writer):
    ...
    while True:
      try:
        ...
        timesteps = metrics["scalar"]["learning/eval_timesteps"]
...
def train_step_2_2(state_low, state_high, data, rng, scheduler_state, is_high_noise=None, scheduler=None, config=None):
  ...
  Timestep Distributions:
    - High-noise expert (coarse structure): samples continuous latent time u in [u_boundary, 1.0],
      where u_boundary = S^{-1}(boundary_ratio), and applies Flow Match time shift S(u) so that
      the resulting timesteps cover [boundary_ratio * num_train_timesteps, num_train_timesteps],
      matching the exact high-noise regime used during inference.
    - Low-noise expert (fine details): samples continuous latent time u in [0, u_boundary],
  ...
  def compute_loss_low(low_params, s_low):
    u = jax.random.uniform(timestep_rng, (bsz,), minval=0.0, maxval=u_boundary)
    t_shifted = (u * shift) / (1.0 + (shift - 1.0) * u)
    timesteps = t_shifted * num_train_timesteps
...
def eval_step_2_2(state_low, state_high, data, rng, scheduler_state, scheduler, config):
  ...
  """
  batch_size_to_eval = getattr(config, "global_batch_size_to_train_on", None)
  if isinstance(batch_size_to_eval, int):
    data = {k: v[:batch_size_to_eval] for k, v in data.items()}
  num_train_timesteps = scheduler.config.num_train_timesteps

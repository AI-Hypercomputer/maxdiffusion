...
def eval_at_checkpoint(
    ...
    if config.pre_compile:
        mesh_configs = ["eval_step", ici_data_parallelism, ici_fsdp_parallelism, ici_tensor_parallelism]
        target_path = os.path.join(config.base_output_directory, "x".join(mesh_configs))
    try: 
...
def train(config):
    ...
    if config.pre_compile:
        ...
        output_path = os.path.join(config.base_output_directory, "x".join(mesh_configs))
        save_compiled(p_train_step, config.run)
    ...
    for checkpoint in eval_checkpoints:
        ...
        max_logging.log(f"checkpoints at {}")
        if config.write_metrics:
            if jax.process_index() == 0:

  ...
  def multistep_dpm_solver_second_order_update(
    ...
    if self.config.algorithm_type == "dpmsolver++":
      ...
      if self.config.solver_type == "midpoint":
        x_t = (
            (sigma_t / sigma_s0) * sample - (alpha_t * (jnp.exp(-h) - 1.0)) * D0 - 0.5 * (alpha_t * (jnp.exp(-h) - 1.0)) * D1
        )
      ...
      if self.config.solver_type == "midpoint":
        x_t = (alpha_t / alpha_s0) * sample - (sigma_t * (jnp.exp(h) - 1.0)) * D0 - 0.5 * (sigma_t * (jnp.exp(h) - 1.0)) * D1
      elif self.config.solver_type == "heun":
        x_t = (

...
from maxdiffusion.utils import is_scipy_available
...
def linear_quadratic_schedule_jax(num_steps: int, threshold_noise: float = 0.025, linear_steps: Optional[int] = None) -> jnp.ndarray:
    if num_steps == 1:
...
class RectifiedFlowSchedulerState:
  ...
  @classmethod
  def create(   #need to change this!
      cls,
...
class FlaxRectifiedFlowMultistepScheduler(FlaxSchedulerMixin, ConfigMixin):
  ...
  def get_initial_timesteps_jax(
        ...
        if self.config.sampler == "Uniform":
            ...
            return time_shift_jax(
                shift, 1.0, jnp.linspace(1.0, 1.0 / num_timesteps, num_timesteps, dtype=self.dtype)
            ).astype(self.dtype)
        else:
            # This should be caught by __init__ but for safety
  ...
  def shift_timesteps_jax(self, samples_shape: Tuple[int, ...], timesteps: jnp.ndarray) -> jnp.ndarray:
        if self.config.shifting == "SD3":
            return sd3_resolution_dependent_timestep_shift_jax(
                samples_shape, timesteps, self.config.target_shift_terminal
            )
        elif self.config.shifting == "SimpleDiffusion":
            return simple_diffusion_resolution_dependent_timestep_shift_jax(
                samples_shape, timesteps, self.config.base_resolution
            )
        return timesteps
    ...
    if pretrained_model_path.is_file():
        ...
        if not diffusers_noise_scheduler_config_path.is_file():
            raise FileNotFoundError(
                f"Scheduler config not found at {diffusers_noise_scheduler_config_path}"
            )
  ...
  def set_timesteps(
        ...
        if timesteps is None:
            num_inference_steps = jnp.minimum(
                self.config.num_train_timesteps, num_inference_steps
            )
            timesteps = self.get_initial_timesteps_jax(
                num_inference_steps, shift=self.config.shift
            ).astype(self.dtype)
  ...
  def step(
        ...
        timestep: jnp.ndarray, # Can be global or per-token, but for RF it's typically global.
        sample: jnp.ndarray,
        ...
        if timestep.ndim == 0: 
            idx = jnp.searchsorted(timesteps_padded, timestep - t_eps, side='right')
            current_t_idx = jnp.where(state.timesteps == timestep, size=1, fill_value=len(state.timesteps))[0][0]
            lower_timestep = jnp.where(current_t_idx + 1 < len(timesteps_padded),
                                       timesteps_padded[current_t_idx + 1],
                                       0.0)

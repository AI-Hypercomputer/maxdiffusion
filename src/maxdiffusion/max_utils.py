...
def create_device_mesh(config, devices=None, logging=True):
    ...
    if multi_slice_env:
        ...
        mesh = mesh_utils.create_hybrid_device_mesh(
            ici_parallelism, dcn_parallelism, devices
        )
    else:
...
def init_train_state(
    model, tx, weights_init_fn, params=None, training=True, eval_only=False
):
    """
...
def get_abstract_state(model, tx, config, mesh, weights_init_fn, training=True):
    ...
    unboxed_sharded_abstract_state = unbox_logicallypartioned_trainstate(
...
def create_learning_rate_schedule(
    learning_rate, learning_rate_schedule_steps, warmup_steps_fraction, max_train_steps
):
...
def calculate_model_tflops(
    module: module_lib.Module, rngs: Union[PRNGKey, RNGSequences], train, **kwargs
):
    ...
    for c in calls:
        ...
        if c.path in visited_paths:
            ...
            if isinstance(c.module, nn.Dense):
                total_flops += 2 * (
                    reduce(lambda x, y: x * y, inputs.shape) * c.module.features
                )
            ...
            # attention_values * v : 2 * batch_size * seq_length_1 * seq_length_2 * heads * head_dim where (heads * head_dim) == hidden_dim
                ...
                qk_einsum = (
                    2
                    * (reduce(lambda x, y: x * y, inputs[0].shape))
                    * inputs[1].shape[1]
                )
                scaling = inputs[0].shape[0] * inputs[0].shape[1] * inputs[1].shape[1]
                softmax = (
                    inputs[0].shape[0] * inputs[0].shape[1] * np.log(inputs[1].shape[1])
                )
                att_v = (
                    2
                    * (reduce(lambda x, y: x * y, inputs[0].shape))

...
def get_flash_block_sizes(config):
  ...
  if len(config.flash_block_sizes.keys()) > 0:
    ...
    if attention_uses_tokamax_ring and "heads_per_tile" in user_block_sizes:
      block_q = user_block_sizes.get("block_q_dkv", user_block_sizes.get("block_q", user_block_sizes.get("block_kv")))
      block_kv = user_block_sizes.get("block_kv")
      block_kv_compute = user_block_sizes.get("block_kv_compute")
      return CustomFlashBlockSizes(
          block_q=block_q,
          block_kv=block_kv,
          block_kv_compute=block_kv_compute,
          block_q_dkv=user_block_sizes.get("block_q_dkv", block_q),
          block_kv_dkv=user_block_sizes.get("block_kv_dkv", block_kv),
          block_kv_dkv_compute=user_block_sizes.get("block_kv_dkv_compute", block_kv_compute),
          block_q_dq=None,
          block_kv_dq=None,

...
def make_laion400m_train_iterator(
  ...
  train_ds = (
    tf.data.Dataset.list_files(os.path.join(config.train_data_dir,"*"), shuffle=True, seed=config.seed)
      ...
      .map(lambda x, y: tf.py_function(partial_tokenize, inp=[x, y], Tout=(tf.bfloat16, tf.bfloat16)), num_parallel_calls=AUTOTUNE)
...
def get_shaped_batch(config, pipeline):
  ...
  shaped_batch["input_ids"] = jax.ShapeDtypeStruct(batch_ids_shape, jnp.bfloat16)
  return shaped_batch
...
def call_pipeline(config, pipeline, num_inference_steps=None, return_timings=False):
  ...
  num_images = (
      int(num_devices * config.per_device_batch_size)
      if config.per_device_batch_size > 0
      else int(config.global_batch_size_to_train_on)
  )
  return pipeline(
      [config.prompt] * num_images,
...
def run(config, commit_hash=None):
  ...
  max_logging.log(
      f"Num steps: {config.num_inference_steps}, height: {config.height}, width: {config.width}," f" images: {num_images}"
  )

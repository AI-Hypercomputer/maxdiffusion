...
class VaeImageProcessor(ConfigMixin):
  ...
  def preprocess(
    ...
    if isinstance(image, supported_formats):
      ...
      raise ValueError(
          f"Input is in incorrect format: {[type(i) for i in image]}. Currently, "
          "we only support {', '.join(supported_formats)}"
      )
    ...
    if image.min() < 0 and do_normalize:
      warnings.warn(
          ...
          "The expected value range for image tensor is [0,1] when passing as pytorch "
          "tensor or numpy Array. You passed `image` with value range "
  ...
  def postprocess(
    ...
    if output_type not in ["latent", "pt", "np", "pil"]:
      deprecation_message = (
          f"the output_type {output_type} is outdated and has been set to `np`. "
          "Please make sure to set it to one of these instead: "
          "`pil`, `np`, `pt`, `latent`"
      )

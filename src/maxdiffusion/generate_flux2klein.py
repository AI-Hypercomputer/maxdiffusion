...
def main(argv):
    ...
    if config.batch_size == 1 and config.ici_tensor_parallelism == 1 and jax.device_count() > 1:
        print(f"ℹ️ Auto-configuring Tensor Parallelism: ici_tensor_parallelism={jax.device_count()}, ici_fsdp_parallelism=1 for batch_size=1 on {jax.device_count()} TPU devices.")
    ...
    with jax.default_device(cpu_device):
        with mesh, nn_partitioning.axis_rules(config.logical_axis_rules):
            ...
            vae_params = jax.tree_util.tree_map(unbox_fn, vae_params, is_leaf=lambda k: isinstance(k, flax_spmd.LogicallyPartitioned))
            vae_params = flax.core.unfreeze(vae_params)
            
            qwen3_params = jax.tree_util.tree_map(unbox_fn, qwen3_params, is_leaf=lambda k: isinstance(k, flax_spmd.LogicallyPartitioned))
            ...
            if config.weights_dtype == "bfloat16":
                ...
                cast_dict_to_bfloat16_inplace(params, device=cpu_device)
                cast_dict_to_bfloat16_inplace(vae_params, device=cpu_device)
                cast_dict_to_bfloat16_inplace(qwen3_params, device=cpu_device)
            ...
            print("\n" + "="*80)
            print("🚀 Pinning all parameters to TPU HBM permanently...")
            print("="*80 + "\n")
            print("Putting params on TPU HBM...")
            with mesh, nn_partitioning.axis_rules(config.logical_axis_rules):
                try:
                    ...
                    k_p = set(flat_p.keys())
                    k_s = set(flat_s.keys())
                    print("Keys in sharding spec but missing in params:", k_s - k_p)
                    print("Keys in params but missing in sharding spec:", k_p - k_s)
                    sys.stdout.flush()
                    raise err
                print("Putting vae_params on TPU HBM...")
                vae_params = jax.device_put(vae_params, vae_shardings)
                print("Putting qwen3_params on TPU HBM...")
                qwen3_params = jax.device_put(qwen3_params, qwen3_shardings)
            print("All parameters placed on TPU HBM successfully!")
            gc.collect()
            jax.effects_barrier()
    ...
    if getattr(config, "interactive", False):
        ...
        main_time = (
            main_trace.get("prompt_encoding", 0.0)
            + main_trace.get("denoise_loop", 0.0)
            + main_trace.get("vae_decode", 0.0)
        )

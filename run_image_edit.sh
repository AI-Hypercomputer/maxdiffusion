...
# ==============================================================================
# Advanced XLA / TPU Runtime Optimization Flags
# ==============================================================================
...
# Detect python binary (cloud-devkit tpu_python or system python3)
if [ -f "/home/ameypasarkar/cloud-devkit/tpu_python" ]; then
  PYTHON_BIN="/home/ameypasarkar/cloud-devkit/tpu_python"
elif [ -f "/home/ameypasarkar_google_com/cloud-devkit/tpu_python" ]; then
...
TPU_VISIBLE_DEVICES=0 $PYTHON_BIN src/maxdiffusion/generate_flux2klein.py \
  ...
  prompt="zoom in on this picture and show a grand entrance to a big ballroom from the front door" \
  image_paths="['${SCRIPT_DIR}/src/maxdiffusion/test_output/test_sdxl.png']" \
  output_dir="${SCRIPT_DIR}/src/maxdiffusion/test_output/" \
  output_name=edited_ballroom_9b.png \
  height=1024 \
  width=1024 \
  ...
  flash_block_sizes='{"block_q": 1024, "block_kv": 1024, "block_kv_compute": 1024}' \
  seed=42
